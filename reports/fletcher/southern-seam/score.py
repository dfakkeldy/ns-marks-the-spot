...
parser.add_argument("--north-reference-dir", required=True, type=Path)
...
INPUT_HASHES = json.loads((R / "southern-seam/observations.json").read_text())[
    "fit_hashes"
]
INPUT_HASHES["southern-seam/observations.json"] = (
    "65d849761da27859346556960d13e4ddf638024b728f21c8a9ffd5e3de12ec50"
)
fresh = json.loads((R / "southern-seam/fresh-checks.json").read_text())
INPUT_HASHES["southern-seam/revised-judique.json"] = fresh["fit_sha256"]
INPUT_HASHES["southern-seam/fresh-checks.json"] = c.digest(
    R / "southern-seam/fresh-checks.json"
)
INPUT_HASHES["southern-seam/longpoint-trace.json"] = c.digest(
    R / "southern-seam/longpoint-trace.json"
)
for relative, expected in INPUT_HASHES.items():
    c.verified(R / relative, expected)
for receipt in json.loads((R / "sheet22-corridor/reference-receipts.json").read_text()):
    c.verified(args.reference_dir / (receipt["name"] + ".geojson"), receipt["sha256"])
d = json.loads((R / "southern-seam/observations.json").read_text())
out = args.out
gcps = {}
for k, f in [
    ("22", "sheet22-corridor/revised-observations.json"),
    ("19", "southern-seam/revised-judique.json"),
    ("19-base", "visual-expansion/sheet-observations.json"),
]:
    control_args = []
...
north_receipt = json.loads(
    (R / "placement-pilot/road-context-receipts.json").read_text()
)[0]
c.verified(args.north_reference_dir / "highways.geojson", north_receipt["sha256"])
features += json.loads((args.north_reference_dir / "highways.geojson").read_text())[
    "features"
]
segments = []
for f in features:
...
for p in d["points"] + fresh["points"]:
...
curves = {}
for p in d["lines"] + [
    json.loads((R / "southern-seam/longpoint-trace.json").read_text())
]:
...
s, t = curves["L22"], curves["L19"]
lo = max(s[:, 1].min(), t[:, 1].min())
hi = min(s[:, 1].max(), t[:, 1].max())
assert hi >= lo, f"No source-content overlap: gap {lo - hi} projected metres"
...
fresh_errors = [
    p["error_ground_m"] for p in scores["points"] if p["id"].startswith("V19")
]
scores["fresh_validation"] = {
    "median_ground_m": float(np.median(fresh_errors)),
    "worst_ground_m": float(max(fresh_errors)),
    "passes": bool(np.median(fresh_errors) <= 100 and max(fresh_errors) <= 200),
    "note": "P19 is consumed as S01; only V19N/V19S are fresh for the local Judique revision.",
}
