...
const directory = process.argv[2] ?? 'reports/fletcher/sheet23/';
...
const options = { pixelSize: { width: 10741, height: 7635 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-23-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-23-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-23-validation-review.csv', 'utf8'), options);
