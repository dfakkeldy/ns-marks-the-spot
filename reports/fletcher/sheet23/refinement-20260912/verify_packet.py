...
old = j(P / 'repaired-fit.json')
assert fit['points'][:12] == old['points'] and len(fit['points']) == 14
assert len(old["points"]) == 12
for added in j(D/'reviewed-additions.json')['points']:
    before=next(p for p in j(P/'validation.json')['points'] if p['id']==added['promoted_from'])
    assert added['pixel_xy']==before['pixel_xy'] and added['lonlat']==before['lonlat']
import struct
assert j(P/'source-receipt.json')['native_dimensions'] == [10741,7635]
assert list(struct.unpack('>II',(DATA/'native/sheet23.png').read_bytes()[16:24])) == [10741,7635]
...
assert freeze['prior_fit_sha256'] == sha(P / 'repaired-fit.json')
assert {p['id'] for p in checks['points']} == {'F01','F03'}
for p in checks['points']:
    ...
    for prior in P.glob('*.json'):
        data=j(prior)
        for q in (data.get('points',[]) if isinstance(data,dict) else []):
            assert p['pixel_xy'] != q.get('pixel_xy') and p['lonlat'] != q.get('lonlat')
for score_name, check_name, fit_path in [('validation-scores.json','validation.json',D/'reviewed-fit.json'),('baseline-scores.json','baseline-checks.json',P/'repaired-fit.json'),('raster-receipt.json','diagnostic-checks.json',D/'reviewed-fit.json')]:
...
for name in ['reviewed-additions.json','fresh-candidates.json','fresh-adjusted.json','validation.json']:
    for point in j(D/name)['points']:
        if 'modern_objectid' in point:
            g = features[point['modern_objectid']]['geometry']
            parts = [g['coordinates']] if g['type']=='LineString' else g['coordinates']
            xy = parts[point.get('modern_part',point.get('modern_part_index'))][point.get('modern_vertex',point.get('modern_vertex_index'))]
...
for framefile in D.glob('*/frames.json'):
    for frame in j(framefile):
        ...
        assert x1-x0 == (700 if 'wide' in framefile.parent.name else 220)
        frame_count += 1
assert frame_count == 16
receipt = j(D/'raster-receipt.json');raster=receipt['raster']
assert sha(Path(raster['path'])) == raster['sha256']
assert receipt['orientation']['samples'] == 71701 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (14,6,2)
...
assert visual['count'] == len(visual['figures']) == 36
...
result={'passed':True,'unchanged_prior_controls':12,'verified_native_frames':16,'viewed_figures':36,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'Two local checks after Grand Lake and Shaw Lake repairs; both fail and coverage remains limited.','whole_sheet_geographically_accepted':False}
