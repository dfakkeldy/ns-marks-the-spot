...
assert fit['points'][:11] == old['points'] and len(fit['points']) == 12
assert fit['prior_control_count'] == 11
assert fit['source_width'] == 10832 and fit['source_height'] == 7582
...
assert receipt['orientation']['samples'] == 71854 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (12,6,2)
...
assert visual['count'] == len(visual['figures']) == 28
...
result={'passed':True,'unchanged_prior_controls':11,'verified_native_frames':6,'viewed_figures':28,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'Two new coastal/island checks near prior controls; no fresh independent central inland validation.','whole_sheet_geographically_accepted':False}
