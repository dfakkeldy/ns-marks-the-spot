...
const directory = process.argv[2] ?? 'reports/fletcher/sheet15/refinement-20260912/';
...
const options = { pixelSize: { width: 10832, height: 7582 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-15-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-15-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-15-validation-review.csv', 'utf8'), options);
