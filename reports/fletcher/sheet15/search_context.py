...
ROOT=Path('/Users/dfakkeldy/Downloads/fletcher-sheet15/search')
ROOT.mkdir(parents=True,exist_ok=True)
DATA=Path('/Users/dfakkeldy/Downloads/fletcher-sheet15/reference-full')
REPO=Path('/Users/dfakkeldy/.codex/worktrees/0ca7/ns-marks-the-spot')
d=json.loads((REPO/'tools/fletcher/observations/sheet-15.json').read_text())
known=[{'pixel_xy':[p['pixel_x'],p['pixel_y']], 'lonlat':[d['meridians'][p['meridian_index']]['lon'],d['parallels'][p['parallel_index']]['lat']]} for p in d['intersections']]
...
im=Image.open('/Users/dfakkeldy/Downloads/fletcher-sheet15/native/sheet15.png');c=im.crop(box);c.save(ROOT/(name+'-source.png'));d=ImageDraw.Draw(c)
...
for feature in json.loads((DATA/'water-polygons.geojson').read_text())['features']:
 ...
 for poly in polys:
  for ring in poly:water_edges.append(xy(ring))
...
ax.set(xlim=(box[0],box[2]),ylim=(box[3],box[1]),title='Modern water / roads: printed-graticule guide ONLY; not physical controls',xlabel='Approximate source x',ylabel='Approximate source y');ax.set_aspect('equal');ax.grid(alpha=.2);fig.tight_layout();fig.savefig(ROOT/(name+'-modern.png'),dpi=100)
