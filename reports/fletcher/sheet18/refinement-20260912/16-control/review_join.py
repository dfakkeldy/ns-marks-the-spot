...
RASTER = DATA / "refinement-20260912/16-control/sheet-18-full-sheet.tif"
NEIGHBORS = {
 "15": ("Sheet 15: experimental 12 controls", Path.home()/"Downloads/fletcher-sheet15/refinement-20260912/sheet-15-full-sheet.tif"),
 "17": ("Sheet 17: preferred provisional 10 controls", Path.home()/"Downloads/fletcher-sheet17/regional-ten/sheet-17-full-sheet.tif"),
 "19": ("Sheet 19: provisional full sheet", Path.home()/"Downloads/judique-full-sheet/result/judique-full-sheet.tif"),
 "21": ("Sheet 21: provisional 15 controls", Path.home()/"Downloads/fletcher-sheet21/repaired-fifteen/sheet-21-full-sheet.tif"),
}
REGIONS = {'15-northwest':[-61.21,45.91,-61.1,45.945], '15-northeast':[-61.05,45.91,-60.93,45.945], '19-west-north':[-61.245,45.85,-61.205,45.905], '19-west-south':[-61.24,45.75,-61.20,45.81], '17-east-north':[-60.875,45.85,-60.825,45.915], '17-east-south':[-60.875,45.75,-60.825,45.80], '21-southwest':[-61.20,45.73,-61.10,45.765], '21-southeast':[-60.98,45.73,-60.87,45.765]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 18: experimental 16 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"refinement-20260912/16-control/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
...
if len(sys.argv)>1 and (OUT / "frames.json").exists():
    previous=json.loads((OUT / "frames.json").read_text());frames=[f for f in previous if f["image"].split(".")[0] not in sys.argv[1:] and not f["image"].startswith("14-")]+frames
