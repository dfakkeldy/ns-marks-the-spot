...
const directory = process.argv[2] ?? 'reports/fletcher/sheet18/refinement-20260912/';
...
const options = { pixelSize: { width: 10832, height: 7683 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-18-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-18-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-18-validation-review.csv', 'utf8'), options);
