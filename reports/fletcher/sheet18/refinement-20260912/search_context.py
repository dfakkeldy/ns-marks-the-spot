...
ROOT=Path('/Users/dfakkeldy/Downloads/fletcher-sheet18/refinement-20260912/search')
ROOT.mkdir(parents=True,exist_ok=True)
DATA=Path('/Users/dfakkeldy/Downloads/fletcher-sheet18/reference-full')
REPO=Path.cwd()
d=json.loads((REPO/'reports/fletcher/sheet18/prior-native-fit.json').read_text())
known=d['points'] # Preserved original four-point affine search only; graticule guide was rejected.
...
im=Image.open('/Users/dfakkeldy/Downloads/fletcher-sheet18/native/sheet18.png');c=im.crop(box);c.save(ROOT/(name+'-source.png'));d=ImageDraw.Draw(c)
...
ax.set(xlim=(box[0],box[2]),ylim=(box[3],box[1]),title='Modern water / roads: preserved four-control affine search guide ONLY; not physical controls',xlabel='Approximate source x',ylabel='Approximate source y');ax.set_aspect('equal');ax.grid(alpha=.2);fig.tight_layout();fig.savefig(ROOT/(name+'-modern.png'),dpi=100)
print('nodes',len(local))

for kind in ['grid','modern']:
 im=Image.open(ROOT/(name+'-'+kind+'.png'));im.thumbnail((2100,2100));im.convert('RGB').save(ROOT/(name+'-'+kind+'.jpg'),quality=92)
