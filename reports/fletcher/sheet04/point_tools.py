...
D=Path('reports/fletcher/sheet04');L=Path.home()/'Downloads/fletcher-sheet04';obs=json.loads((D/'search-guide.json').read_text());p=[dict(pixel_xy=[a['pixel_x'],b['pixel_y']],lonlat=[a['lon'],b['lat']]) for a in obs['meridians'] for b in obs['parallels']];g=np.linalg.lstsq(np.c_[[r['lonlat'] for r in p],np.ones(len(p))],np.array([r['pixel_xy'] for r in p]),rcond=None)[0]
rows=[]
