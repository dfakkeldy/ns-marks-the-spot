"""Inspect the actual provisional Sheet04 raster, alone and overlaid with NSTDB geometry.
...
DATA = Path.home() / "Downloads/fletcher-sheet04"
OUT = HERE / "adjacent-sheet-review"
OUT.mkdir(exist_ok=True)
RASTER = DATA / "refinement-20260912/11-control/sheet-04-full-sheet.tif"
NEIGHBORS = {"02": ("Sheet 02: provisional nine controls", Path.home()/"Downloads/fletcher-sheet02/regional-nine/sheet-02-full-sheet.tif"), "05": ("Sheet 05: provisional twelve controls", Path.home()/"Downloads/fletcher-sheet05/refinement-20260912/sheet-05-full-sheet.tif"), "07": ("Sheet 07: earlier eleven-control baseline", Path.home()/"Downloads/fletcher-sheet07/regional-eleven/sheet-07-full-sheet.tif")}
REGIONS = {"02-north-land": [-60.49,46.775,-60.32,46.82], "02-north-sea": [-60.33,46.775,-60.07,46.82], "05-west-north": [-60.51,46.70,-60.43,46.80], "05-west-south": [-60.51,46.60,-60.43,46.71], "07-south-coast": [-60.50,46.59,-60.30,46.65], "07-south-sea": [-60.32,46.59,-60.05,46.65]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 04: experimental 11 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"refinement-20260912/11-control/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
    ...
    frames.append(
        {
            ...
            "reference": "reports/fletcher/sheet04/reference-receipts.json",
