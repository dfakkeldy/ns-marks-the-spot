...
RASTER = DATA / "regional-eight/sheet-04-full-sheet.tif"
NEIGHBORS = {"05": ("Sheet 5: provisional 11-control TPS", Path.home()/"Downloads/fletcher-sheet05/regional-eleven/sheet-05-full-sheet.tif"),"07": ("Sheet 7: provisional 11-control TPS", Path.home()/"Downloads/fletcher-sheet07/regional-eleven/sheet-07-full-sheet.tif")}
REGIONS = {"05-west-north": [-60.51,46.70,-60.43,46.80], "05-west-south": [-60.51,46.60,-60.43,46.71], "07-south-coast": [-60.50,46.59,-60.30,46.65], "07-south-sea": [-60.32,46.59,-60.05,46.65]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 04: provisional 8 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-eight/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
