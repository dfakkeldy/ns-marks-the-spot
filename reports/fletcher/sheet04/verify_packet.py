...
D=Path('reports/fletcher/sheet04');L=Path.home()/'Downloads/fletcher-sheet04'
...
def digest(p):
 ...
 return h.hexdigest()
s=json.loads((D/'source-receipt.json').read_text());assert digest(Path(s['source_path']))==s['source_sha256'];assert digest(L/'native/sheet04.tif')==s['tif_sha256'];assert digest(L/'native/manifest.json')==s['manifest_sha256'];assert Image.open(s['source_path']).size==tuple(s['native_dimensions'])
...
paths=[Path(r['raster']['path']),Path.home()/'Downloads/fletcher-sheet05/regional-eleven/sheet-05-full-sheet.tif',Path.home()/'Downloads/fletcher-sheet07/regional-eleven/sheet-07-full-sheet.tif'];write(D/'join-provenance.json',dict(rasters=[dict(path=str(p),sha256=digest(p)) for p in paths],status='Four actual western Sheet5 and southern Sheet7 comparisons personally inspected. Black is locally closer but western drainage differs; southern coast has offset/tilt. Sea-only placement is not geographic seam evidence. No seam accepted. Northern Sheet2 pending; eastern region is sea.'))
...
write(D/'browser-verification.json',dict(method='Actual GeoTIFF UI import in isolated Chromium via Playwright; desktop 1440x1000 and mobile 390x844 screenshots personally inspected.',geographic_acceptance=False,url='http://127.0.0.1:4198/?basemap=osm&taxSale=off&layers=modern&position=46.70,-60.34,11',raster_sha256=before['storedRasterSha256'],raster_bytes=before['rasterBytes'],pixel_size=before['record']['pixelSize'],georef=before['record']['georef'],preview_size=before['previewSize'],transparent_preview_pixels=before['transparentPixels'],opaque_preview_pixels=before['opaquePixels'],raster_bytes_and_georef_preserved_on_reload=True,enabled_state_survived_reload=proof['enabledStateSurvived'],console_and_page_errors=errors,screenshots=[dict(path=str(b/n),sha256=digest(b/n)) for n in ['import-desktop.png','reload-desktop.png','reload-mobile.png']],full_receipt=str(b/'verification.json')))
write(D/'boundary-review-frame.json',dict(source_sha256=s['source_sha256'],native_dimensions=s['native_dimensions'],rotation_degrees=0,image='boundary-overview.jpg',extension_native_boxes=[],status='Full native overview, corners, coordinate labels and complete boundary personally inspected. One continuous ring retains all mapped coastal and sea regions, interior legend, Ingonish Island and offshore rocks. No mapped extensions outside main frame identified.'))
context=D/'matching-context';context.mkdir(exist_ok=True);frames=[]
for name,box in [('north',[1190,980,4650,3400]),('central',[1190,3000,4350,5100]),('south',[1190,4800,4350,6480])]:
 for kind in ['grid','modern']:shutil.copyfile(L/f'search/{name}-{kind}.jpg',context/f'{name}-{kind}.jpg')
 frames.append(dict(name=name,native_box=box,source_image=name+'-grid.jpg',modern_image=name+'-modern.jpg',rotation_degrees=0,method='Eight Cartesian crossings reconstructed from prior meridian/parallel coordinates; only native latitude and outer longitude labels rechecked in this pass. These give search guidance only, not physical fitting controls or warped-image proof.'))
...
write(D/'visual-review.json',dict(personally_inspected_point_frames=count,personally_inspected_warped_frames=8,personally_inspected_adjacent_frames=4,personally_inspected_browser_screenshots=3,geographic_acceptance=False,limitations=['Three fresh checks fail numerical thresholds: median 125.757995703 m and worst 219.577753759 m','Substantial northern/interior river displacement and unsupported southern drainage','Broad eastern sea is unsupported extrapolation','Two initial checks remain separate diagnostics; no repair or promotion performed','Western Sheet5 and southern Sheet7 seams unaccepted; northern Sheet2 pending']))
print('Verified source, TIFF, manifest, four references, two original grid files, eight unchanged controls, frozen checks, raster, coverage, browser and',count,'point frames.')
