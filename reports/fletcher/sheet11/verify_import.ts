...
const directory = process.argv[2] ?? 'reports/fletcher/sheet11/';
const expectedControls = JSON.parse(readFileSync(directory + 'coastal-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'coastal-checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10771, height: 7551 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-11-provisional-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-11-provisional-review.csv', 'utf8'), options);
...
const scores = JSON.parse(readFileSync(directory + 'coastal-tps-scores.json', 'utf8'));
