...
const directory = process.argv[2] ?? 'reports/fletcher/sheet11/refinement-20260910/';
const expectedControls = JSON.parse(readFileSync(directory + 'central-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'central-checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10771, height: 7551 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-11-refined-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-11-refined-review.csv', 'utf8'), options);
...
const scores = JSON.parse(readFileSync(directory + 'central-scores.json', 'utf8'));
