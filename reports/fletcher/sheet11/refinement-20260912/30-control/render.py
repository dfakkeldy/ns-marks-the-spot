"""Replay the existing GDAL pipeline over both unchanged Sheet11 components."""
...
spec = importlib.util.spec_from_file_location('pipeline', HERE.parents[2] / 'full-sheets/render.py')
pipeline = importlib.util.module_from_spec(spec)
spec.loader.exec_module(pipeline)
c = pipeline.c
out = Path.home() / 'Downloads/fletcher-sheet11/refinement-20260912/30-control'
out.mkdir(parents=True, exist_ok=True)
source = Path.home() / 'Downloads/fletcher-sheet11/native/sheet11.png'
fit_path = HERE / 'repaired-fit.json'
...
for component in ['mainland', 'island']:
    path = HERE.parents[1] / f'boundary-{component}.json'
...
c.write(HERE / 'raster-receipt.json', dict(source_sha256=c.digest(source), fit_sha256=c.digest(fit_path), script_sha256=c.digest(Path(__file__)), boundaries=boundaries, cutline_sha256=c.digest(cutline), orientation=orientation, command=command, raster=dict(path=str(tif), sha256=c.digest(tif), size=info['size'], bounds=info['wgs84Extent']), geographic_status='Provisional; fresh validation failed. Full mainland extension and Sea Wolf Island retained.'))
print(tif)
