...
const directory = process.argv[2] ?? 'reports/fletcher/sheet11/refinement-20260912/';
const expectedControls = JSON.parse(readFileSync(directory + 'repaired-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'reused-checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10771, height: 7551 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-11-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-11-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-11-validation-review.csv', 'utf8'), options);
...
const scores = [...JSON.parse(readFileSync(directory + 'diagnostic-scores.json', 'utf8')).points,
