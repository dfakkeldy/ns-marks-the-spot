...
**Reviewed placement, September 6, 2026: eight approximate points and four group areas. The church point is corrected east of Highway 19; the other eleven geometries follow reviewed marks on the original sheet. Another 121 transcribed annotations await source-mark review.**

Download [mapped-annotations.geojson](mapped-annotations.geojson) and load it through the web map's shared file drop zone (it appears under Your data). The [mapping queue](mapping-queue.json) retains the remaining IDs and original label rectangles. Teal marks in the modern comparison panels show the exported geometry. For the church, the historical panel keeps the original source mark while the modern panel shows the corrected location and a grey X at the rejected warp prediction. This is a research preview using Judique's supported-area draft, not a fully accepted sheet or surveyed historical-site layer.
...
| 015 R.C. Church | Supported mark; modern placement corrected | Local review places the church east of Highway 19. OpenStreetMap node 1325630467 supplies the corrected approximate point; the old west-side warp prediction is retained as rejected. A 1968 history supports site continuity; exact footprint unresolved. |
...
- **Teal mapped geometry:** the reviewed symbol point or source-group outline passed through the frozen TPS, except the locally corrected church point in the modern panel. Group edges are sampled at intervals no greater than 10 native pixels before transformation. The area bounds ambiguous printed marks, not the historical property, mill grounds or positional error. Lettering centres and operational search windows never become exported feature geometry.
...
## Church road-side correction

Local review on September 6 identified the church as east of Highway 19. The original TPS point at longitude -61.49137305, latitude 45.87732223 is on the west side and is rejected as the church preview location. [OpenStreetMap node 1325630467](https://www.openstreetmap.org/node/1325630467), version 2, identifies St. Andrew's Catholic Parish Church at longitude -61.4906198, latitude 45.8787475. This modern point is approximately 169 ground metres north-northeast of the old prediction and east of the recorded NSTDB Route 19 segment (OBJECTID 156236). It supplies an approximate replacement backed by the user's road-side correction and the historical site-continuity account; it does not establish the former building's precise footprint.

The correction and versioned modern reference are recorded in `locality-review.json`, `pilot.json` and the church GeoJSON properties. Its exported status is `locally-reviewed-approximate`; `map_derived_geometry` preserves the rejected original prediction. The native source anchor, frozen controls, all other eleven exported geometries and 133 null inventory/site geometries remain unchanged. A rebuild reapplies this correction. Modern church reference: [© OpenStreetMap contributors, ODbL 1.0](https://www.openstreetmap.org/copyright); this is separate from the Fletcher imagery's licence.
