...
D=Path(__file__).resolve().parent;ROOT=D.parents[3];DATA=Path.home()/'Downloads/fletcher-sheet10';EXT=DATA/'refinement-20260912'
...
assert f['points'][:12]==r(D.parent/'repaired-fit.json')['points'] and len(f['points'])==13
...
assert h(DATA/'native/sheet10.png')==f['source_sha256']==freeze['source_sha256']
assert h(D.parent/'boundary.json')==freeze['boundary_sha256']
p=f['points'][-1];assert p['id']=='C15'
assert p['promotion']['old_point']==next(q for q in r(D.parent/'validation.json')['points'] if q['id']=='V04')
assert p['promotion']['old_result']==next(q for q in r(D.parent/'validation-scores.json')['points'] if q['id']=='V04')
...
for kind,extra in [('controls',[]),('diagnostic-review',diag['points']),('validation-review',v['points'])]:
 rows=list(csv.DictReader((D/f'sheet-10-{kind}.csv').open()));assert len(rows)==len(f['points'])+len(extra)
...
i=r(D/'import-verification.json');assert (i['controls'],i['checks'],i['validation_checks'])==(13,3,2) and i['semantic_csv_roundtrip'] and i['maximum_web_gdal_difference_projected_m']<.001
receipt={'passed':True,'controls':13,'reused_checks':3,'collected_fresh_checks':2,'old_twelve_controls_unchanged':True,'promoted_pixels_coordinates_and_failure_preserved':True,'figure_count':len(figures),'figures':figures,'raster':artifact,'scope':'Computational, provenance and import verification only. Geography fails; F01 has post-score topology uncertainty; repair not recommended as replacement.'}
