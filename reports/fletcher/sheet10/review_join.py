...
RASTER = DATA / "regional-twelve/sheet-10-full-sheet.tif"
NEIGHBORS = {"11": ("Sheet 11: provisional 26-control TPS", Path.home() / "Downloads/fletcher-sheet11/completion-20260911/sheet-11-exact.tif"), "12": ("Sheet 12: provisional 14-control TPS", Path.home() / "Downloads/fletcher-sheet12/regional-fourteen/sheet-12-full-sheet.tif")}
REGIONS = {"11-west-north": [-60.875,46.37,-60.83,46.445], "11-west-south": [-60.875,46.28,-60.83,46.35], "12-south-west": [-60.84,46.255,-60.68,46.28], "12-south-east": [-60.66,46.255,-60.48,46.28]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 10: provisional 12 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-twelve/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
