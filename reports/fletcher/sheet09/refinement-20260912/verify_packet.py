...
D=Path(__file__).resolve().parent;ROOT=D.parents[3];DATA=Path.home()/'Downloads/fletcher-sheet09';EXT=DATA/'refinement-20260912'
...
assert f['points'][:24]==r(D.parent/'completion-20260911/reviewed-fit.json')['points'] and len(f['points'])==25
...
assert h(DATA/'native/sheet09.png')==f['source_sha256']==freeze['source_sha256']
assert h(D.parent/'expansion-20260910/boundary.json')==freeze['boundary_sha256']
p=f['points'][-1];assert p['id']=='C25'
assert p['promotion']['old_point']==next(q for q in r(D.parent/'completion-20260911/reviewed-diagnostic-checks.json')['points'] if q['id']=='Q04')
for key in ['pixel_xy','lonlat']:
 assert p[key]==p['promotion']['old_point'][key]
 assert not {tuple(p[key]) for p in f['points']} & {tuple(p[key]) for p in v['points']+diag['points']}
for fit,checks,scores in [(D/'repaired-fit.json',D/'validation.json',D/'validation-scores.json'),(D/'repaired-fit.json',D/'reused-checks.json',D/'raster-receipt.json'),(D.parent/'completion-20260911/reviewed-fit.json',D/'baseline-validation.json',D/'baseline-validation-scores.json')]:
...
for kind,extra in [('controls',[]),('diagnostic-review',diag['points']),('validation-review',v['points'])]:
 rows=list(csv.DictReader((D/f'sheet-09-{kind}.csv').open()));assert len(rows)==len(f['points'])+len(extra)
...
assert f['points'][-1]['promotion']['old_result']==next(q for q in r(D.parent/'completion-20260911/regional-scores.json')['points'] if q['id']=='Q04')
assert {q['id'] for q in r(D/'rejected-before-score.json')['points']}=={'F03','F05'}
...
i=r(D/'import-verification.json');assert (i['controls'],i['checks'],i['validation_checks'])==(25,7,2) and i['semantic_csv_roundtrip'] and i['maximum_web_gdal_difference_projected_m']<.001
receipt={'passed':True,'controls':25,'reused_checks':7,'collected_fresh_checks':2,'old_twenty_four_controls_unchanged':True,'new_control_matches_reviewed_prior_check':True,'figure_count':len(figures),'figures':figures,'raster':artifact,'scope':'Computational, provenance and import verification only. Fresh thresholds and whole-sheet geography fail. Local coastal improvement only; Q01, V02 and R01 identity concerns remain; F03/F05 rejected before scoring.'}
