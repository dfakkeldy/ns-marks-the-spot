...
const directory = process.argv[2] ?? 'reports/fletcher/sheet09/expansion-20260910/';
const expectedControls = JSON.parse(readFileSync(directory + 'draft-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'draft-checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10762, height: 7642 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-09-draft-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-09-draft-review.csv', 'utf8'), options);
...
const scores = JSON.parse(readFileSync(directory + 'draft-scores.json', 'utf8'));
