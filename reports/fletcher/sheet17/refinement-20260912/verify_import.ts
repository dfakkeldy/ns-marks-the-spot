...
const directory = process.argv[2] ?? 'reports/fletcher/sheet17/refinement-20260912/';
...
const options = { pixelSize: { width: 10873, height: 7642 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-17-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-17-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-17-validation-review.csv', 'utf8'), options);
