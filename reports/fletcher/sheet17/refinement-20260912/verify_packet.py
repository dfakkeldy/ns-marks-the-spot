...
assert fit['points'][:10] == old['points'] and len(fit['points']) == 11
assert len(old["points"]) == 10
assert fit['source_dimensions'] == [10873,7642]
...
for score_name, check_name, fit_path in [('validation-scores.json','validation.json',D/'reviewed-fit.json'),('baseline-scores.json','baseline-checks.json',P/'regional-fit.json'),('raster-receipt.json','diagnostic-checks.json',D/'reviewed-fit.json')]:
...
for name in ['reviewed-additions.json','fresh-candidates.json','fresh-adjusted.json','fresh-final.json','validation.json']:
...
assert receipt['orientation']['samples'] == 71795 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (11,4,1)
...
assert visual['count'] == len(visual['figures']) == 31
...
result={'passed':True,'unchanged_prior_controls':10,'verified_native_frames':12,'viewed_figures':31,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'One new island check within a controlled lake system, failed; no independent whole-interior validation.','whole_sheet_geographically_accepted':False}
