...
const directory = process.argv[2] ?? 'reports/fletcher/sheet24/';
...
const options = { pixelSize: { width: 10782, height: 7655 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-24-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-24-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-24-validation-review.csv', 'utf8'), options);
