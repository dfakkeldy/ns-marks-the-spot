...
assert fit['points'][:14] == old['points'] and len(fit['points']) == 16
assert len(old["points"]) == 14
...
import struct
assert j(P/'source-receipt.json')['native_dimensions'] == [10782,7655]
assert list(struct.unpack('>II',(DATA/'native/sheet24.png').read_bytes()[16:24])) == [10782,7655]
...
assert {p['id'] for p in checks['points']} == {'F01','F02'}
...
assert receipt['orientation']['samples'] == 73110 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (16,5,2)
...
assert visual['count'] == len(visual['figures']) == 29
...
result={'passed':True,'unchanged_prior_controls':14,'verified_native_frames':8,'viewed_figures':29,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'Two local checks after Hadley Beach and McPherson Lake repairs; median fails and coverage remains limited.','whole_sheet_geographically_accepted':False}
