...
const directory = process.argv[2] ?? 'reports/fletcher/mabou-full-sheet/north-audit-20260909/candidate36/';
const expectedControls = JSON.parse(readFileSync(directory + 'fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10822, height: 7531 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-16-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-16-review.csv', 'utf8'), options);
...
const scores = JSON.parse(readFileSync(directory + 'scores.json', 'utf8'));
