...
for prefix,fit,diagnostics,validation in [('thirteen-', 'repaired-fit.json','reused-checks.json','validation.json'),('', 'final-fit.json','final-diagnostic.json','final-validation.json')]:
 ...
 for kind,extra in [('controls',[]),('diagnostic-review',json.loads((D/diagnostics).read_text())['points']),('validation-review',json.loads((D/validation).read_text())['points'])]:
  with (D/f'{prefix}sheet-13-{kind}.csv').open('w',newline='') as f:
