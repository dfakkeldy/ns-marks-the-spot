...
const directory = process.argv[2] ?? 'reports/fletcher/sheet21/';
...
const options = { pixelSize: { width: 10811, height: 7646 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-21-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-21-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-21-validation-review.csv', 'utf8'), options);
