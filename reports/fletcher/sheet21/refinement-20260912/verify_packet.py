...
assert fit['points'][:15] == old['points'] and len(fit['points']) == 17
assert len(old["points"]) == 15
...
assert fit['source_dimensions'] == [10811,7646]
...
assert receipt['orientation']['samples'] == 71786 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (17,8,2)
...
assert visual['count'] == len(visual['figures']) == 35
...
result={'passed':True,'unchanged_prior_controls':15,'verified_native_frames':12,'viewed_figures':35,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'Two local checks after Cook Lakes and McIntosh island repairs; mainland check fails and coverage remains limited.','whole_sheet_geographically_accepted':False}
