...
const directory = process.argv[2] ?? 'reports/fletcher/sheet20/refinement-20260912/';
const expectedControls = JSON.parse(readFileSync(directory + 'reviewed-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'diagnostic-checks.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10792, height: 7662 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-20-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-20-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-20-validation-review.csv', 'utf8'), options);
