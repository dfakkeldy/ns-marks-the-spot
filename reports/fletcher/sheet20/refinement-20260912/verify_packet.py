...
P = D.parent
R = D.parents[3]
DATA = Path.home() / 'Downloads/fletcher-sheet20'
def j(p):
...
assert freeze['prior_fit_sha256'] == sha(P / 'regional-fit.json')
assert {p['id'] for p in checks['points']} == {'F02','F03'}
...
for score_name, check_name, fit_path in [('validation-scores.json','validation.json',D/'reviewed-fit.json'),('baseline-scores.json','baseline-checks.json',P/'regional-fit.json'),('diagnostic-scores.json','diagnostic-checks.json',D/'reviewed-fit.json')]:
...
for framefile in D.glob('*/frames.json'):
    for frame in j(framefile):
        ...
        assert x1-x0 == (700 if framefile.parent.name in ['repair-native-wide','fresh-wide'] else 220)
        frame_count += 1
assert frame_count == 15
receipt = j(D/'raster-receipt.json');raster=receipt['raster']
assert sha(Path(raster['path'])) == raster['sha256']
assert sum(o['samples'] for o in receipt['orientation']) == 72292 and all(o['nonnegative_determinants'] == 0 for o in receipt['orientation'])
assert receipt['script_sha256'] == sha(D/'render.py')
verify(receipt['boundaries'])
...
assert coverage['cutline_sha256'] == sha(DATA/'refinement-20260912/cutline.geojson')
...
assert visual['count'] == len(visual['figures']) == 37
...
result={'passed':True,'unchanged_prior_controls':12,'verified_native_frames':15,'viewed_figures':37,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'Two new inlet island tips pass local limits; no independent whole-interior validation.','whole_sheet_geographically_accepted':False}
