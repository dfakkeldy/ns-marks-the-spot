"""Inspect the actual provisional Sheet12 raster, alone and overlaid with NSTDB geometry.
...
RASTER = DATA / "refinement-20260912/17-control/sheet-12-full-sheet.tif"
RASTERS = [
    ("Previous 14-control TPS", DATA / "regional-fourteen/sheet-12-full-sheet.tif"),
    ("New 17-control TPS + modern reference", RASTER),
]
REGIONS = {'northwest-rivers': [-60.86, 46.195, -60.72, 46.27], 'baddeck-forks': [-60.82, 46.13, -60.71, 46.2], 'christopher-big-glen': [-60.735, 46.205, -60.65, 46.26], 'st-anns-monroe': [-60.665, 46.218, -60.55, 46.273], 'seal-otter': [-60.58, 46.2, -60.46, 46.25], 'baddeck-kidston': [-60.85, 46.08, -60.69, 46.145], 'port-bevis': [-60.66, 46.11, -60.58, 46.18], 'island-point': [-60.61, 46.11, -60.5, 46.17], 'kemp-coffin-extension': [-60.7, 46.063, -60.58, 46.12], 'lockman-extension': [-60.55, 46.067, -60.465, 46.12], 'full-sheet': [-60.87, 46.06, -60.45, 46.28]}
...
for layer in ["water-lines", "roads"]:
    vectors[layer] = []
    for f in json.loads((DATA / f"refinement-20260912/reference/{layer}.geojson").read_text())[
        "features"
    ]:
        g = f["geometry"]
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"refinement-20260912/17-control/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
    ...
    frames.append(
        {
            ...
            "reference": "reports/fletcher/sheet12/refinement-20260912/south-reference-receipts.json",
