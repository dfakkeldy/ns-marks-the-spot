...
const directory = process.argv[2] ?? 'reports/fletcher/sheet12/refinement-20260912/';
const expectedControls = JSON.parse(readFileSync(directory + 'final-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'final-diagnostic.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10801, height: 7713 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-12-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-12-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-12-validation-review.csv', 'utf8'), options);
const expectedValidation = JSON.parse(readFileSync(directory + 'final-validation.json', 'utf8')).points.length;
...
const scores = [...JSON.parse(readFileSync(directory + 'final-diagnostic-scores.json', 'utf8')).points,
  ...JSON.parse(readFileSync(directory + 'final-validation-scores.json', 'utf8')).points];
