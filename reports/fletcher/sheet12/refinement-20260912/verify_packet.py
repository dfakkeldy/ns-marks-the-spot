...
D=Path(__file__).resolve().parent;ROOT=D.parents[3];DATA=Path.home()/'Downloads/fletcher-sheet12'
read=lambda p:json.loads(p.read_text())
h=lambda p:hashlib.file_digest(p.open('rb'),'sha256').hexdigest()
old=read(D.parent/'repaired-fit.json');six=read(D/'repaired-fit.json');final=read(D/'final-fit.json')
assert six['points'][:14]==old['points'] and final['points'][:16]==six['points']
assert len(final['points'])==17
...
assert h(DATA/'native/sheet12.png')==final['source_sha256']
assert h(D.parent/'boundary.json')==read(D/'final-freeze.json')['boundary_sha256']
for fit,checks,scores in [('repaired-fit.json','validation.json','validation-scores.json'),('final-fit.json','final-validation.json','final-validation-scores.json'),('final-fit.json','final-diagnostic.json','final-diagnostic-scores.json')]:
...
for prefix,fit,diag,val in [('sixteen-',six,read(D/'repair-diagnostic.json'),read(D/'validation.json')),('',final,read(D/'final-diagnostic.json'),read(D/'final-validation.json'))]:
 for kind,extra in [('controls',[]),('diagnostic-review',diag['points']),('validation-review',val['points'])]:
  rows=list(csv.DictReader((D/f'{prefix}sheet-12-{kind}.csv').open()));points=fit['points']+extra;assert len(rows)==len(points)
...
for r in read(D/'south-reference-receipts.json'):
 for key in ['south','prior','combined']:assert h(Path(r[key+'_path']))==r[key+'_sha256']
figures=[]
...
for name in ['coverage.json','final-coverage.json']:assert read(D/name)['passed'] and read(D/name)['transparent_interior_cells']==0
assert read(D/'import-verification.json')['controls']==17
receipt={'passed':True,'final_controls':17,'fresh_checks':2,'reused_checks':7,'source_sha256':final['source_sha256'],'final_fit_sha256':h(D/'final-fit.json'),'complete_boundary_sha256':h(D.parent/'boundary.json'),'previous_fourteen_controls_unchanged':True,'promotions_preserve_old_pixels_coordinates_and_scores':True,'figure_count':len(figures),'figures':figures,'raster_experiments':artifacts,'scope':'Provenance, computational and rendering verification. Geographic acceptance remains false.'}
