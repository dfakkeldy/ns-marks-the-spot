...
const directory = process.argv[2] ?? 'reports/fletcher/sheet08/refinement-20260912/';
...
const options = { pixelSize: { width: 10812, height: 7622 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-08-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-08-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-08-validation-review.csv', 'utf8'), options);
...
const scores = [...JSON.parse(readFileSync(directory + 'raster-receipt.json', 'utf8')).points,
