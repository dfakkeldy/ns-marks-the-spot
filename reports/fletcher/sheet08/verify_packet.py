...
D=Path('reports/fletcher/sheet08');L=Path.home()/'Downloads/fletcher-sheet08'
...
def digest(p):
 ...
 return h.hexdigest()
s=json.loads((D/'source-receipt.json').read_text());assert digest(Path(s['source_path']))==s['source_sha256'];assert digest(L/'native/sheet08.tif')==s['tif_sha256'];assert digest(L/'native/manifest.json')==s['manifest_sha256'];assert Image.open(s['source_path']).size==tuple(s['native_dimensions'])
...
f=json.loads((D/'repaired-fit.json').read_text());old=json.loads((D/'reviewed-fit.json').read_text());h=digest(D/'repaired-fit.json');assert f['points'][:9]==old['points'];assert digest(D/'reviewed-fit.json')==json.loads((D/'initial-freeze.json').read_text())['fit_sha256'];assert h==json.loads((D/'repair-freeze.json').read_text())['fit_sha256']
for qid,cid in [('Q01','C12')]:
...
write(D/'preservation-verification.json',dict(verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),initial_nine_controls_unchanged=True,promotions=['Q01 to C12'],promotion_coordinates_unchanged=True,failed_initial_fit_and_scores_preserved=True,fresh_validation_disjoint_from_fit=True,original_two_prior_files_unchanged=True,source_sha256_verified=s['source_sha256'],reference_files_verified=len(refs),fit_sha256=h))
...
paths=[Path(r['raster']['path']),Path.home()/'Downloads/fletcher-sheet09/completion-20260911/sheet-09-full-sheet.tif',Path.home()/'Downloads/fletcher-sheet10/regional-twelve/sheet-10-full-sheet.tif'];write(D/'join-provenance.json',dict(rasters=[dict(path=str(p),sha256=digest(p)) for p in paths],status='Four actual western Sheet9 and southern Sheet10 comparisons personally inspected. Large western gap and southern offset/overlap. No seam accepted. Northern Sheet5 and eastern Sheet7 pending.'))
...
write(D/'browser-verification.json',dict(method='Actual GeoTIFF UI import in isolated Chromium via Playwright; desktop 1440x1000 and mobile 390x844 screenshots personally inspected.',geographic_acceptance=False,url='http://127.0.0.1:4198/?basemap=osm&taxSale=off&layers=modern&position=46.54,-60.66,11',raster_sha256=before['storedRasterSha256'],raster_bytes=before['rasterBytes'],pixel_size=before['record']['pixelSize'],georef=before['record']['georef'],preview_size=before['previewSize'],transparent_preview_pixels=before['transparentPixels'],opaque_preview_pixels=before['opaquePixels'],raster_bytes_and_georef_preserved_on_reload=True,enabled_state_survived_reload=proof['enabledStateSurvived'],console_and_page_errors=errors,screenshots=[dict(path=str(b/n),sha256=digest(b/n)) for n in ['import-desktop.png','reload-desktop.png','reload-mobile.png']],full_receipt=str(b/'verification.json')))
...
for name,box in [('northwest',[1444,940,4600,3700]),('northcentral',[4350,940,7200,3700]),('northeast',[6950,950,9650,3850]),('southwest',[1460,3500,5900,6420]),('southeast',[5700,3600,9660,6440])]:
...
write(D/'visual-review.json',dict(personally_inspected_point_frames=count,personally_inspected_warped_frames=11,personally_inspected_adjacent_frames=4,personally_inspected_browser_screenshots=3,geographic_acceptance=False,limitations=['Three fresh checks: median 154.800614 m and worst 344.071022 m exceed 100/200 m thresholds','Sparse southwest and central interior support; no stable reservoir-shore acceptance','Substantial historical/modern river differences and post-survey reservoir development','Reused Q03 remains 131.195863 m away','Western Sheet9 and southern Sheet10 joins unaccepted; northern Sheet5 and eastern Sheet7 pending','Unsupported areas remain extrapolation']))
print('Verified source, TIFF, manifest, four references, two original grid files, nine preserved controls, one promotion, frozen checks, raster, coverage, browser and',count,'point frames.')
