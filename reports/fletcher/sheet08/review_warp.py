...
RASTER = DATA / "regional-ten/sheet-08-full-sheet.tif"
RASTERS = [
    ("10-control provisional TPS raster", RASTER),
    ("Same raster + modern reference", RASTER),
]
REGIONS = {'north-branch': [-60.86, 46.55, -60.76, 46.625], 'northeast-margaree': [-60.85, 46.5, -60.71, 46.59], 'coulmeach': [-60.86, 46.48, -60.7, 46.55], 'first-second-forks': [-60.86, 46.43, -60.69, 46.5], 'upper-ingonish': [-60.69, 46.565, -60.58, 46.63], 'ingonish-mckinnon': [-60.58, 46.57, -60.47, 46.63], 'reservoir-interior': [-60.74, 46.46, -60.57, 46.58], 'indian-brook': [-60.62, 46.44, -60.53, 46.51], 'french-little-rivers': [-60.56, 46.44, -60.47, 46.55], 'gisborne-lake': [-60.61,46.53,-60.51,46.59], 'full-sheet': [-60.9, 46.42, -60.44, 46.65]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-ten/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
