...
D=Path('reports/fletcher/sheet08');L=Path.home()/'Downloads/fletcher-sheet08';obs=json.loads((D/'search-guide.json').read_text());p=[dict(pixel_xy=[a['pixel_x'],a['pixel_y']],lonlat=[obs['meridians'][a['meridian_index']]['lon'],obs['parallels'][a['parallel_index']]['lat']]) for a in obs['intersections']];g=np.linalg.lstsq(np.c_[[r['lonlat'] for r in p],np.ones(len(p))],np.array([r['pixel_xy'] for r in p]),rcond=None)[0]
rows=[]
