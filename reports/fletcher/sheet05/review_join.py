...
RASTER = DATA / "regional-eleven/sheet-05-full-sheet.tif"
NEIGHBORS = {"06": ("Sheet 6: provisional 10-control TPS", Path.home()/"Downloads/fletcher-sheet06/regional-ten/sheet-06-full-sheet.tif"),"08": ("Sheet 8: provisional 10-control TPS", Path.home()/"Downloads/fletcher-sheet08/regional-ten/sheet-08-full-sheet.tif")}
REGIONS = {"06-west-north": [-60.91,46.70,-60.80,46.80], "06-west-south": [-60.91,46.60,-60.80,46.71], "08-south-west": [-60.87,46.59,-60.65,46.65], "08-south-east": [-60.67,46.59,-60.43,46.65]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 05: provisional 11 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-eleven/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
