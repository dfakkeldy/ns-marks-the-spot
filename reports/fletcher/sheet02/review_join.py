...
RASTER = DATA / "regional-nine/sheet-02-full-sheet.tif"
NEIGHBORS = {"03": ("Sheet 3: provisional 7-control TPS", Path.home()/"Downloads/fletcher-sheet03/regional-seven/sheet-03-full-sheet.tif"),"04": ("Sheet 4: provisional 8-control TPS", Path.home()/"Downloads/fletcher-sheet04/regional-eight/sheet-04-full-sheet.tif")}
REGIONS = {'03-west-north': [-60.49, 46.88, -60.425, 46.995], '03-west-south': [-60.49, 46.775, -60.425, 46.89], '04-south-coast': [-60.47, 46.76, -60.26, 46.815], '04-south-sea': [-60.28, 46.76, -60.03, 46.815]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 02: provisional 9 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-nine/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
