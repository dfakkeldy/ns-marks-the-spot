...
D=Path('reports/fletcher/sheet02');L=Path.home()/'Downloads/fletcher-sheet02'
...
def digest(p):
 ...
 return h.hexdigest()
s=json.loads((D/'source-receipt.json').read_text());assert digest(Path(s['source_path']))==s['source_sha256'];assert digest(L/'native/sheet02.tif')==s['tif_sha256'];assert digest(L/'native/manifest.json')==s['manifest_sha256'];assert Image.open(s['source_path']).size==tuple(s['native_dimensions'])
...
f=json.loads((D/'final-fit.json').read_text());old=json.loads((D/'reviewed-fit.json').read_text());h=digest(D/'final-fit.json');assert f['points'][:8]==old['points'];q=json.loads((D/'diagnostic-checks.json').read_text())['points'][0];assert f['points'][-1]['pixel_xy']==q['pixel_xy'] and f['points'][-1]['lonlat']==q['lonlat'];assert digest(D/'reviewed-fit.json')==json.loads((D/'initial-freeze.json').read_text())['fit_sha256'];assert h==json.loads((D/'final-freeze.json').read_text())['fit_sha256']
...
write(D/'preservation-verification.json',dict(verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),initial_eight_controls_unchanged=True,promotions=["Q01 to C09"],promotion_coordinates_unchanged=True,initial_fit_and_scores_preserved=True,fresh_validation_disjoint_from_fit=True,original_two_prior_files_unchanged=True,source_sha256_verified=s['source_sha256'],reference_files_verified=len(refs),fit_sha256=h))
...
write(D/'render-provenance.json',dict(source=s['source_path'],source_sha256=s['source_sha256'],source_native_dimensions=s['native_dimensions'],fit_sha256=h,boundary_sha256=r['boundary_sha256'],raster_sha256=r['raster']['sha256'],render_script='reports/fletcher/full-sheets/render.py',render_script_sha256=digest(Path('reports/fletcher/full-sheets/render.py')),method='One exact GDAL TPS (-et 0), EPSG:3857, 5 projected metre cells, complete mapped frame, RGBA. Initial failed diagnostic preserved with original eight-control fit; promoted Q01 is never a fresh check.'))
paths=[Path(r['raster']['path']),Path.home()/'Downloads/fletcher-sheet03/regional-seven/sheet-03-full-sheet.tif',Path.home()/'Downloads/fletcher-sheet04/regional-eight/sheet-04-full-sheet.tif'];write(D/'join-provenance.json',dict(rasters=[dict(path=str(p),sha256=digest(p)) for p in paths],status='Four actual western Sheet3 and southern Sheet4 comparisons personally inspected; detailed findings in README. No seam accepted. Sheet1 northwest comparison pending.'))
b=L/'regional-nine/browser';proof=json.loads((b/'verification.json').read_text());before,after=proof['before'],proof['after'];assert before['storedRasterSha256']==after['storedRasterSha256']==r['raster']['sha256'];assert before['record']['georef']==after['record']['georef'];assert before['record']['pixelSize']==after['record']['pixelSize'];assert before['transparentPixels']==after['transparentPixels'];errors=json.loads((b/'errors.json').read_text());assert not errors
write(D/'browser-verification.json',dict(method='Actual GeoTIFF UI import in isolated Chromium via Playwright; desktop 1440x1000 and mobile 390x844 screenshots personally inspected.',geographic_acceptance=False,url='http://127.0.0.1:4198/?basemap=osm&taxSale=off&layers=modern&position=46.88,-60.27,11',raster_sha256=before['storedRasterSha256'],raster_bytes=before['rasterBytes'],pixel_size=before['record']['pixelSize'],georef=before['record']['georef'],preview_size=before['previewSize'],transparent_preview_pixels=before['transparentPixels'],opaque_preview_pixels=before['opaquePixels'],raster_bytes_and_georef_preserved_on_reload=True,enabled_state_survived_reload=proof['enabledStateSurvived'],console_and_page_errors=errors,screenshots=[dict(path=str(b/n),sha256=digest(b/n)) for n in ['import-desktop.png','reload-desktop.png','reload-mobile.png']],full_receipt=str(b/'verification.json')))
write(D/'boundary-review-frame.json',dict(source_sha256=s['source_sha256'],native_dimensions=s['native_dimensions'],rotation_degrees=0,image='boundary-overview.jpg',extension_native_boxes=[],status='Full native overview, corners, coordinate labels and complete boundary personally inspected. One continuous ring retains all mapped regions, interior legend, North/Middle/South Pond portions, White Point islands and offshore rocks. No outside mapped extensions found.'))
context=D/'matching-context';context.mkdir(exist_ok=True);frames=[]
for name,box in [('north',[1250,950,5100,4000]),('south',[1250,3800,5100,6510])]:
...
write(D/'visual-review.json',dict(personally_inspected_point_frames=count,personally_inspected_warped_frames=7,personally_inspected_adjacent_frames=4,personally_inspected_browser_screenshots=3,geographic_acceptance=False,limitations=['Two fresh checks pass local thresholds but cover only the Black Head / White Point coast','Broad eastern sea remains unsupported extrapolation','Initial Q01 failed at 322.8776 m and was promoted unchanged; failure preserved','Western Sheet3 and southern Sheet4 joins remain unaccepted; northwest Sheet1 pending']))
print('Verified source, TIFF, manifest, four references, two original grid files, eight preserved controls and one unchanged promotion, frozen checks, raster, coverage, browser and',count,'point frames.')
