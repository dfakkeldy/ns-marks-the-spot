...
const directory = process.argv[2] ?? 'reports/fletcher/sheet02/';
...
assert.equal(checks.gcps.length, 8);
...
const scores = [...JSON.parse(readFileSync(directory + 'initial-diagnostic-tps.json', 'utf8')).points,
...
console.log(JSON.stringify({ controls: expectedControls, initial_diagnostic_controls: 8, checks: expectedChecks, validation_checks: expectedValidation, semantic_csv_roundtrip: true,
