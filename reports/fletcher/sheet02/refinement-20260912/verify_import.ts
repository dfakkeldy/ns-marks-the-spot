...
const directory = process.argv[2] ?? 'reports/fletcher/sheet02/refinement-20260912/';
...
const options = { pixelSize: { width: 10829, height: 7582 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-02-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-02-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-02-validation-review.csv', 'utf8'), options);
