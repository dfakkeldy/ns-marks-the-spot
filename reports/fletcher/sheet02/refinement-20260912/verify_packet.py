...
assert fit['points'][:9] == old['points'] and len(fit['points']) == 10
assert fit['prior_control_count'] == 9
assert fit['source_width'] == 10829 and fit['source_height'] == 7582
...
assert freeze['boundary_sha256'] == sha(P / 'boundary.json')
assert {p['id'] for p in checks['points']} == {'F04'}
for p in checks['points']:
    ...
    for prior in [P/'validation.json', P/'rejected-diagnostics.json', D/'candidate-controls.json']:
        for q in j(prior)['points']:
            assert p['pixel_xy'] != q['pixel_xy'] and p['lonlat'] != q['lonlat']
assert {p['id'] for p in j(D/'rejected-controls.json')['points']} == {'R01'}
assert {p['id'] for p in j(D/'rejected-fresh.json')['points']} == {'F03'}
...
for name in ['candidate-controls.json','reviewed-additions.json','fresh-candidates.json','validation.json']:
...
assert receipt['orientation']['samples'] == 71200 and receipt['orientation']['nonnegative_determinants'] == 0
...
assert (imports['controls'],imports['checks'],imports['validation_checks']) == (10,2,1)
...
result={'passed':True,'unchanged_prior_controls':9,'verified_native_frames':12,'viewed_figures':28,'coverage_and_sampled_orientation':'pass','web_csv_and_solver':'pass','actual_browser_import_desktop_mobile_reload':'pass','fresh_check_scope':'One new coordinate on old V01 island; correlated extent only, no independent inland validation.','whole_sheet_geographically_accepted':False}
