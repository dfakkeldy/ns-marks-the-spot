...
const directory = process.argv[2] ?? 'reports/fletcher/sheet07/';
const expectedControls = JSON.parse(readFileSync(directory + 'repaired-fit.json', 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + 'repair-diagnostic.json', 'utf8')).points.length;
const options = { pixelSize: { width: 10821, height: 7693 } };
const controls = parseFletcherGcps(readFileSync(directory + 'sheet-07-controls.csv', 'utf8'), options);
const checks = parseFletcherGcps(readFileSync(directory + 'sheet-07-diagnostic-review.csv', 'utf8'), options);
const validation = parseFletcherGcps(readFileSync(directory + 'sheet-07-validation-review.csv', 'utf8'), options);
const expectedValidation = JSON.parse(readFileSync(directory + 'fresh-validation.json', 'utf8')).points.length;
...
const scores = [...JSON.parse(readFileSync(directory + 'repair-diagnostic-scores.json', 'utf8')).points,
  ...JSON.parse(readFileSync(directory + 'fresh-validation-scores.json', 'utf8')).points];
...
console.log(JSON.stringify({ controls: expectedControls, checks: expectedChecks, validation_checks: expectedValidation, semantic_csv_roundtrip: true,
