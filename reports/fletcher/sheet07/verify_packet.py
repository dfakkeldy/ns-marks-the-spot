...
D=Path('reports/fletcher/sheet07');L=Path.home()/'Downloads/fletcher-sheet07'
...
def digest(p):
 ...
 return h.hexdigest()
s=json.loads((D/'source-receipt.json').read_text());assert digest(Path(s['source_path']))==s['source_sha256'];assert digest(L/'native/sheet07.tif')==s['tif_sha256'];assert digest(L/'native/manifest.json')==s['manifest_sha256'];assert Image.open(s['source_path']).size==tuple(s['native_dimensions'])
...
for qid,cid in [('V03','C13')]:
 p=next(p for p in json.loads((D/'validation.json').read_text())['points'] if p['id']==qid);q=next(p for p in f['points'] if p['id']==cid);assert p['pixel_xy']==q['pixel_xy'] and p['lonlat']==q['lonlat']
for name in ['fresh-validation.json','repair-diagnostic.json']:
...
write(D/'preservation-verification.json',dict(verified_at_utc=datetime.datetime.now(datetime.timezone.utc).isoformat(),initial_ten_controls_unchanged=True,promotions=['V03 to C13'],promotion_coordinates_unchanged=True,failed_initial_fit_and_scores_preserved=True,fresh_validation_disjoint_from_fit=True,original_two_prior_files_unchanged=True,source_sha256_verified=s['source_sha256'],reference_files_verified=len(refs),fit_sha256=h))
...
paths=[Path(r['raster']['path']),Path.home()/'Downloads/fletcher-sheet08/regional-ten/sheet-08-full-sheet.tif'];write(D/'join-provenance.json',dict(rasters=[dict(path=str(p),sha256=digest(p)) for p in paths],status='Three actual western Sheet8 comparisons personally inspected; seams unaccepted. Northern Sheet4 pending; eastern and southern boundaries are offshore.'))
...
write(D/'browser-verification.json',dict(method='Actual GeoTIFF UI import in isolated Chromium via Playwright; desktop 1440x1000 and mobile 390x844 screenshots personally inspected.',geographic_acceptance=False,url='http://127.0.0.1:4198/?basemap=osm&taxSale=off&layers=modern&position=46.54,-60.43,11',raster_sha256=before['storedRasterSha256'],raster_bytes=before['rasterBytes'],pixel_size=before['record']['pixelSize'],georef=before['record']['georef'],preview_size=before['previewSize'],transparent_preview_pixels=before['transparentPixels'],opaque_preview_pixels=before['opaquePixels'],raster_bytes_and_georef_preserved_on_reload=True,enabled_state_survived_reload=proof['enabledStateSurvived'],console_and_page_errors=errors,screenshots=[dict(path=str(b/n),sha256=digest(b/n)) for n in ['import-desktop.png','reload-desktop.png','reload-mobile.png']],full_receipt=str(b/'verification.json')))
write(D/'boundary-review-frame.json',dict(source_sha256=s['source_sha256'],native_dimensions=s['native_dimensions'],rotation_degrees=0,image='boundary-overview.jpg',extension_native_boxes=[],status='Full native overview, corners, coordinate labels and complete boundary personally inspected. One continuous ring retains all mapped river and lake regions. No mapped extensions outside main frame identified.'))
context=D/'matching-context';context.mkdir(exist_ok=True);frames=[]
for name,box in [('north',[1334,1005,4050,3050]),('central',[1330,2800,3000,4950]),('south',[1322,4650,2550,6500])]:
 for kind in ['grid','modern']:shutil.copyfile(L/f'search/{name}-{kind}.jpg',context/f'{name}-{kind}.jpg')
 frames.append(dict(name=name,native_box=box,source_image=name+'-grid.jpg',modern_image=name+'-modern.jpg',rotation_degrees=0,method='Eight intersections of the prior approximate Cartesian printed-graticule guide give search guidance only, not physical fitting controls or warped-image proof.'))
...
write(D/'visual-review.json',dict(personally_inspected_point_frames=count,personally_inspected_warped_frames=7,personally_inspected_adjacent_frames=3,personally_inspected_browser_screenshots=3,geographic_acceptance=False,limitations=['Two post-repair fresh checks fail: median 311.995500 m, worst 476.977451 m','Controls occupy a narrow western strip; offshore placement is unsupported extrapolation','Historical river/shore geometry differs from modern references','Earlier V03 failed at 760.512277 m and was promoted unchanged, never counted fresh again','Western Sheet8 seams unaccepted; northern Sheet4 pending']))
print('Verified source, TIFF, manifest, four references, two original grid files, ten preserved controls, one promotion, frozen checks, raster, coverage, browser and',count,'point frames.')
