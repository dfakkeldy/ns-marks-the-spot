"""Verify preserved Sheet 7 experiment and evidence, never geographic acceptance."""
from pathlib import Path
import json,hashlib,csv,subprocess
D=Path(__file__).resolve().parent;ROOT=D.parents[3];DATA=Path.home()/'Downloads/fletcher-sheet07';EXT=DATA/'refinement-20260912'
...
assert h(DATA/'native/sheet07.png')==f['source_sha256']==freeze['source_sha256']
assert h(D.parent/'boundary.json')==freeze['boundary_sha256']
p=f['points'][-1];old=next(p for p in r(D.parent/'fresh-validation.json')['points'] if p['id']=='V04')
assert p['id']=='C14' and p['promotion']['old_point']==old
assert p['promotion']['old_result']==next(p for p in r(D.parent/'fresh-validation-scores.json')['points'] if p['id']=='V04')
for key in ['pixel_xy','lonlat']:
 ...
 # Independence includes earlier scored diagnostics/validation, not just fit rows.
 old_checks=[p for name in ['diagnostic.json','validation.json','fresh-validation.json'] for p in r(D.parent/name)['points']]
 assert not {tuple(p[key]) for p in old_checks} & {tuple(p[key]) for p in v['points']}
assert [p['id'] for p in v['points']]==['F08']
correction=r(D/'independence-correction.json');assert correction['new_point']['lonlat']==correction['old_point']['lonlat']
assert correction['new_point']==next(p for p in r(D/'initial-candidate-validation.json')['points'] if p['id']=='F07')
assert {p['id'] for p in r(D/'rejected-before-score.json')['points']}=={'F06','F09'}
for fit,checks,scores in [(D/'repaired-fit.json',D/'validation.json',D/'validation-scores.json'),(D/'repaired-fit.json',D/'reused-checks.json',D/'raster-receipt.json'),(D.parent/'repaired-fit.json',D/'baseline-validation.json',D/'baseline-validation-scores.json'),(D/'repaired-fit.json',D/'initial-candidate-validation.json',D/'initial-candidate-validation-scores.json'),(D.parent/'repaired-fit.json',D/'initial-candidate-baseline-validation.json',D/'initial-candidate-baseline-validation-scores.json')]:
...
for kind,extra in [('controls',[]),('diagnostic-review',diag['points']),('validation-review',v['points'])]:
 rows=list(csv.DictReader((D/f'sheet-07-{kind}.csv').open()));assert len(rows)==len(f['points'])+len(extra)
...
for name in ['repaired-fit.json','boundary.json']:
 raw=subprocess.check_output(['git','show',baseline['base_commit']+':reports/fletcher/sheet07/'+name],cwd=ROOT);assert hashlib.sha256(raw).hexdigest()==h(D.parent/name)
...
proof=r(D/'browser-retry/verification.json');mobile=r(D/'browser-retry/mobile-verification.json')
...
assert r(D/'browser-retry/errors.json')==[]
for p in sorted(D.glob('browser*/*.png')):figures.append({'path':str(p.relative_to(ROOT)),'sha256':h(p)})
i=r(D/'import-verification.json');assert (i['controls'],i['checks'],i['validation_checks'])==(12,5,1) and i['semantic_csv_roundtrip'] and i['maximum_web_gdal_difference_projected_m']<.001
assert r(D/'acceptance.json')['geographic_acceptance'] is False and r(D/'acceptance.json')['recommended_replacement'] is False
receipt={'passed':True,'controls':12,'reused_checks':5,'qualified_fresh_checks':1,'old_eleven_controls_unchanged':True,'new_control_matches_reviewed_prior_check':True,'figure_count':len(figures),'figures':figures,'raster':artifact,'scope':'Provenance, computation, coverage and import only. Twelve-control experiment not recommended as replacement. One qualified fresh check is insufficient; F07 duplicate classification corrected, all results preserved.'}
