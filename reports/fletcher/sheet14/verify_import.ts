...
const directory = process.argv[2] ?? 'reports/fletcher/sheet14/';
const refined = Boolean(process.argv[2]);
const expectedControls = JSON.parse(readFileSync(directory + (refined ? 'fit.json' : 'final-fit.json'), 'utf8')).points.length;
const expectedChecks = JSON.parse(readFileSync(directory + (refined ? 'checks.json' : 'final-checks.json'), 'utf8')).points.length;
...
const scores = JSON.parse(readFileSync(directory + (refined ? 'scores.json' : 'final-scores.json'), 'utf8'));
