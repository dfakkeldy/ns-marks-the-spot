...
RASTERS = [
    ("22 controls (previous)", DATA / "result/sheet-14-full-sheet.tif"),
    (
        "25 controls (refined)",
...
REGIONS = {
    "cove-brook": [-61.415, 46.152, -61.376, 46.186],
    "strathlorne": [-61.328, 46.176, -61.276, 46.213],
    "hay-river": [-61.26, 46.101, -61.222, 46.123],
}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"refinement-20260909/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
    ...
    frames.append(
        {
            ...
            "reference": "../reference-receipts.json",
