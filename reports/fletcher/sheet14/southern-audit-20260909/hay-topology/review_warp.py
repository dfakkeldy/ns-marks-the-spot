...
DATA = Path.home() / "Downloads/fletcher-sheet14"
OUT = HERE / "warped-review"
OUT.mkdir(exist_ok=True)
RASTERS = [
    (
        "25 controls (previous)",
        DATA / "refinement-20260909/result25/sheet-14-full-sheet.tif",
    ),
    (
        "26 controls (southern + Hay correction)",
        DATA / "hay-boundary-audit/hay-result/sheet-14-full-sheet.tif",
    ),
]
REGIONS = {
    "northeast-mabou": [-61.394, 46.095, -61.346, 46.132],
    "hay-river": [-61.273, 46.093, -61.222, 46.125],
}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"hay-boundary-audit/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
    ...
    frames.append(
        {
            ...
            "reference": "reports/fletcher/sheet14/reference-receipts.json",
