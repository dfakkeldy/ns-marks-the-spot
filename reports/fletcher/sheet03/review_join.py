...
RASTER = DATA / "regional-seven/sheet-03-full-sheet.tif"
NEIGHBORS = {"05": ("Sheet 5: provisional 11-control TPS", Path.home()/"Downloads/fletcher-sheet05/regional-eleven/sheet-05-full-sheet.tif"),"06": ("Sheet 6: provisional 10-control TPS", Path.home()/"Downloads/fletcher-sheet06/regional-ten/sheet-06-full-sheet.tif")}
REGIONS = {'05-south-west': [-60.89, 46.765, -60.68, 46.82], '05-south-east': [-60.69, 46.765, -60.43, 46.82], '06-extension': [-60.93, 46.77, -60.81, 46.835]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 03: provisional 7 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"regional-seven/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
