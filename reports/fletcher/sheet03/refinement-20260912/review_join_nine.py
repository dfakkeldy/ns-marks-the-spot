...
OUT = HERE / "adjacent-sheet-review-nine"
OUT.mkdir(exist_ok=True)
RASTER = DATA / "refinement-20260912/result-nine/sheet-03-full-sheet.tif"
NEIGHBORS = {"05": ("Sheet 5: provisional 11-control TPS", Path.home()/"Downloads/fletcher-sheet05/regional-eleven/sheet-05-full-sheet.tif"),"01": ("Sheet 1: provisional 8-control TPS",Path.home()/"Downloads/fletcher-sheet01/regional-eight/sheet-01-full-sheet.tif"),"02": ("Sheet 2: provisional 9-control TPS",Path.home()/"Downloads/fletcher-sheet02/regional-nine/sheet-02-full-sheet.tif"),"06": ("Sheet 6: provisional 10-control TPS", Path.home()/"Downloads/fletcher-sheet06/regional-ten/sheet-06-full-sheet.tif")}
REGIONS = {'01-north-west':[-60.76,46.94,-60.61,46.99],'01-north-east':[-60.61,46.94,-60.44,46.99],'02-east-pond':[-60.53,46.85,-60.43,46.97],'05-south-west': [-60.89, 46.765, -60.68, 46.82], '05-south-east': [-60.69, 46.765, -60.43, 46.82], '06-extension': [-60.93, 46.77, -60.81, 46.835]}
...
for name, (west, south, east, north) in REGIONS.items():
    ...
    RASTERS = [("Sheet 03: refined 9 controls", RASTER), NEIGHBORS[name.split("-")[0]]]
    fig, axes = plt.subplots(1, 2, figsize=(15, 10))
    for i, ((label, raster), ax) in enumerate(zip(RASTERS, axes)):
        dest = DATA / f"refinement-20260912/result-nine/{name}-{i}.png"
        subprocess.run(
            [
                "gdal_translate",
                "-q",
