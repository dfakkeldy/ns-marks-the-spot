...
gdal.UseExceptions()
parser=argparse.ArgumentParser(description='Verify the component masks retain the complete original sheet footprint');parser.add_argument('--artifacts',type=Path,required=True);args=parser.parse_args();here=Path(__file__).resolve().parent;parts=json.loads((here/'components.json').read_text());width,height=json.loads((here/parts['components'][0]['fit_file']).read_text())['source_dimensions'];actual=np.zeros((height,width),np.uint8)
...
ring=parts['source_union_ring_pixel_xy'];path=Polygon(np.array(ring+[ring[0]]));expected=np.zeros(actual.shape,bool)
