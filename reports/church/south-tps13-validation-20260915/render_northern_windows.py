...
R=Path(__file__).resolve().parent;E=R/'northern-expansion';C=Path('/Users/dfakkeldy/Downloads/church-south-road-validation-20260915');freeze=json.loads((R/'freeze.json').read_text());raster=Path(freeze['raster']);assert hashlib.sha256(raster.read_bytes()).hexdigest()==freeze['raster_sha256'];cs=load_gcps(R/'controls.csv')
points=[json.loads((R/f'observations/IS{i}.json').read_text()) for i in [39,40]]
features=[];refs=[]
