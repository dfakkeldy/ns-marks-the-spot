...
for ll,branches in nodes.items():
 ...
 if len(branches)>=3 and len(names)>=1 and -61.3<ll[0]<-60.94 and 45.60<ll[1]<45.73:rows.append(dict(lonlat=ll,branches=branches))
...
(r/'all-junction-search.json').write_text(json.dumps(rows,indent=2));print('Candidates:',len(rows))
