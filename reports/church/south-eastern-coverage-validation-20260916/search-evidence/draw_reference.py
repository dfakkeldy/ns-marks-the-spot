...
r=Path('/Users/dfakkeldy/Downloads/church-south-eastern-coverage-validation-20260916');names=json.loads((r/'coastal-names.geojson').read_text());water=Path('/Users/dfakkeldy/Downloads/church-georeferencing-20260912/reference/water-lines.geojson');roads=Path('/Users/dfakkeldy/Downloads/church-south-road-validation-20260915');font=ImageFont.truetype('/System/Library/Fonts/Helvetica.ttc',13)
for name,label in [('maciver','MacIvers Point'),('malagawatch','Malagawatch Point')]:
 q=next(f for f in names['features'] if f['properties']['geoname']==label and f['properties']['concise_ds']=='Cape');lon,lat=q['geometry']['coordinates'];c=math.cos(math.radians(lat));span=2400;im=Image.new('RGB',(1100,1140),'white');d=ImageDraw.Draw(im);selected=[];near=[]
