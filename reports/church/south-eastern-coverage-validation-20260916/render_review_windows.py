...
points=[json.loads((R/('observations/IS52-corrected.json' if i==52 else f'observations/IS{i}.json')).read_text()) for i in [52,53]]
features=[];refs=[]
...
for p,q in zip(points,pred,strict=True):
 ...
 d.rectangle((0,1000,1000,1060),fill='#fffdf5');d.text((10,1006),p['id']+(' corrected-reference diagnostic' if p['id']=='IS52' else ' fresh check')+'; actual TPS13 raster; 2000 ground-metre window',fill='black');d.text((10,1023),'Orange X: warped source. Red +: reference. Blue: water. Brown/black: roads/highways.',fill='black');d.text((10,1040),'David Rumsey / Stanford - CC BY-NC-SA 3.0. Reference: original NSTDB.',fill='black')
