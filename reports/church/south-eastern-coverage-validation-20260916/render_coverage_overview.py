...
for p in load_gcps(r/'corrected-validation.csv'):
 col,row=gdal.ApplyGeoTransform(inv,*lonlat_to_mercator(p.lon,p.lat));x=col/d.RasterXSize*width;y=row/d.RasterYSize*height;color='#cc7700' if p.label=='IS52' else '#bc2828' if p.role=='check' else '#17589a'
