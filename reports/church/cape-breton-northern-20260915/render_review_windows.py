...
C=Path('/Users/dfakkeldy/Downloads/church-cape-breton-northern-20260915')
R=C/'rendered/cape-breton-tps8-review-20m.tif'
receipt=json.loads((H/'selected-tps8/artifact-receipt.json').read_text())
assert hashlib.sha256(R.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(H/'selected-tps8/controls.csv');control_ids={p.label for p in cs}
...
for p,pred in zip(points,preds,strict=True):
 lon,lat=p['lonlat'];x,y=lonlat_to_mercator(lon,lat);half_ground=2000 if p['id']=='CB15' else 1000;half=half_ground/math.cos(math.radians(lat));bounds=[x-half,y-half,x+half,y+half]
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh after TPS8 freeze' if p['id'] in ['CB15','CB16','CB17'] else 'selection diagnostic'
 d.rectangle((0,1000,1000,1040),fill='#fffdf5');d.text((10,1007),f"{p['id']} - {role} - actual TPS8 / original NSTDB - {half_ground*2} ground-metre window",fill='black');d.text((10,1023),'Orange: warped source. Red: reference. Rumsey / Stanford - CC BY-NC-SA 3.0.',fill='black');dest=out/f"{p['id']}.jpg";im.convert('RGB').save(dest,quality=92)
