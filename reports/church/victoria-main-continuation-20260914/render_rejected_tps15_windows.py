...
R=C/'tps15-rendered/victoria-main-tps15-review-20m.tif'
receipt=json.loads((H/'harbour-support/artifact-receipt.json').read_text())
assert hashlib.sha256(R.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(H/'harbour-support/controls.csv');control_ids={p.label for p in cs}
...
gdal.UseExceptions();ds=gdal.Open(str(R));inverse=gdal.InvGeoTransform(ds.GetGeoTransform());out=H/'rejected-tps15-warped-review';out.mkdir(exist_ok=True)
...
for p,pred in zip(points,preds,strict=True):
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh after TPS15 freeze' if p['id'] in ['VM27','VM28'] else 'selection diagnostic'
