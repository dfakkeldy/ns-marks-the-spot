"""Render the frozen fourteen-control TPS and audit sampled orientation."""
...
SOURCE_SHA='81a694ed1cd7a9b3b7d77c4b16dd947a984932214397f319cab072375f695621'
def digest(p):
...
def main():
 ...
 cs=load_gcps(HERE/'controls.csv');assert len(cs)==14 and all(p.role=='control' for p in cs);args=build_gcp_arguments(cs);boundary=HERE.parent/'content-boundary.json';assert digest(boundary)==freeze['boundary_sha256'];ring=json.loads(boundary.read_text())['ring_pixel_xy'];poly=Cutline(tuple(map(tuple,ring)))
 ...
 cut=a.out/'victoria-main-corrected-tps14-cutline.geojson';cut.write_text(cutline_geojson(transform(densify(poly,250)).tolist(),3857));vrt=a.out/'victoria-main-corrected-tps14.vrt'
 run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Victoria main; fourteen physical controls; TPS; no geographic acceptance.',*args,a.source.resolve(),vrt)
 raster=a.out/'victoria-main-corrected-tps14-review-20m.tif'
