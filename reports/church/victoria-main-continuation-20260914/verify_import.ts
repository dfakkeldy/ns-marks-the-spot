...
const root='reports/church/victoria-main-continuation-20260914/';
const results=[];
for(const [file,checks] of [['controls.csv',0],['diagnostic-review.csv',6],['fresh-validation.csv',1]] as const){
 const p=parseFletcherGcps(readFileSync(root+'reference-correction/'+file,'utf8'),{pixelSize:{width:33711,height:31468}});
 assert.equal(p.gcps.length,14);assert.equal(p.checks.length,checks);assert.deepEqual(parseFletcherGcps(serializeFletcherGcps(p)).rows,p.rows);
 ...
 assert.ok(worst<.001);results.push({file,controls:14,checks,semanticRoundtrip:true,maxTpsDifferenceProjectedMetres:worst});
}
