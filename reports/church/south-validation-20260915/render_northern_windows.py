...
points=[p for p in points if p["id"] in ["IS25","IS26"]]
...
gdal.UseExceptions();ds=gdal.Open(str(raster));inv=gdal.InvGeoTransform(ds.GetGeoTransform());out=R/'northern-expansion/warped-review';out.mkdir(parents=True,exist_ok=True)
