"""Inspect the four new eastern/southern physical checks on the unchanged actual affine raster."""
...
R=Path(__file__).resolve().parent;E=R/'eastern-expansion';C=Path('/Users/dfakkeldy/Downloads/church-south-road-validation-20260915');freeze=json.loads((R/'freeze.json').read_text());raster=Path(freeze['raster']);assert hashlib.sha256(raster.read_bytes()).hexdigest()==freeze['raster_sha256'];cs=load_gcps(R/'controls.csv')
points=[json.loads((R/f'observations/IS{i}.json').read_text()) for i in range(30,34)]
features=[];refs=[]
...
water={}
for p in [Path('/Users/dfakkeldy/Downloads/church-georeferencing-20260912/reference/water-lines.geojson'),Path('/Users/dfakkeldy/Downloads/church-review-20260913/south-west-water-lines.geojson'),Path('/Users/dfakkeldy/Downloads/church-south-eastern-validation-20260915/interior-water.geojson')]:
 refs.append(p)
...
pred=subprocess.check_output(['gdaltransform','-order','1',*build_gcp_arguments(cs)],input=''.join(f"{p['pixel_xy'][0]} {p['pixel_xy'][1]}\n" for p in points),text=True);pred=[list(map(float,s.split()[:2])) for s in pred.splitlines()];reviews=[]
for p,q in zip(points,pred,strict=True):
 ...
 d.rectangle((0,1000,1000,1060),fill='#fffdf5');d.text((10,1006),p['id']+' fresh check; actual affine4 raster; 2000 ground-metre window',fill='black');d.text((10,1023),'Orange X: warped source. Red +: reference. Blue: water. Brown/black: roads/highways.',fill='black');d.text((10,1040),'David Rumsey / Stanford - CC BY-NC-SA 3.0. Reference: original NSTDB.',fill='black')
