"""Inspect the three new road checks on the unchanged actual affine raster."""
...
R=Path(__file__).resolve().parent;E=R/'road-expansion';C=Path('/Users/dfakkeldy/Downloads/church-south-road-validation-20260915');freeze=json.loads((R/'freeze.json').read_text());raster=Path(freeze['raster']);assert hashlib.sha256(raster.read_bytes()).hexdigest()==freeze['raster_sha256'];cs=load_gcps(R/'controls.csv')
points=[json.loads((R/f'observations/IS{i}.json').read_text()) for i in range(27,30)]
features=[];refs=[]
...
water={}
for p in [Path('/Users/dfakkeldy/Downloads/church-georeferencing-20260912/reference/water-lines.geojson'),Path('/Users/dfakkeldy/Downloads/church-review-20260913/south-west-water-lines.geojson')]:
 refs.append(p)
