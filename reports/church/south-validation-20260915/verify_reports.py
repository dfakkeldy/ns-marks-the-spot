"""Replay preserved first validation, original geometry and frozen input receipts."""
...
R=Path(__file__).resolve().parent;E=R/'northern-expansion';ROAD=R/'road-expansion';EAST=R/'eastern-expansion';ROOT=R.parents[2];O=R.parent/'physical-review-20260913/inverness-south';s=importlib.util.spec_from_file_location('score',R.parent/'target-refinement-20260913/score_models.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
...
cs=load_gcps(R/'controls.csv');base=load_gcps(ROOT/'tools/church/gcps/inverness-south.csv');rows=load_gcps(R/'fresh-validation.csv');assert [p for p in rows if p.role=='control']==cs;checks=[p for p in rows if p.role=='check'];assert [p.label for p in checks]==[f'IS{i}' for i in range(20,34)];assert not {p.label for p in checks}&set(f['prior_selection_features']);assert len({(p.lon,p.lat) for p in cs+checks})==18
...
east_published=read(EAST/'previous-published.json');east_commit=east_published['input_commit']
subprocess.run(['git','merge-base','--is-ancestor',east_commit,'origin/nightly'],check=True)
for record in east_published['stable_files']:
 path=R/record['path'];assert digest(path)==record['sha256']
 assert path.read_bytes()==subprocess.check_output(['git','show',east_commit+':'+str(path.relative_to(ROOT))])
assert (R/'fresh-validation.csv').read_bytes().startswith(subprocess.check_output(['git','show',east_commit+':'+east_published['previous_csv_path']]))
assert (R/'snapshots/first-ten-accuracy-summary.json').read_bytes()==subprocess.check_output(['git','show',east_commit+':'+east_published['previous_accuracy_path']])
interior_receipt=read(EAST/'interior-water-receipt.json');interior=Path(interior_receipt['path']);assert digest(interior)==interior_receipt['sha256']
interior_rows=read(interior)['features'];interior_ids={v['properties']['OBJECTID'] for v in interior_rows};assert len(interior_ids)==len(interior_rows)==interior_receipt['count']==1455
assert interior_ids==set(read(EAST/'interior-water-id-audit.json')['objectIds'])
...
for q in checks:
 ...
 first=read(R/f'{q.label}-first.json');assert digest(path)==first['observation_sha256'];assert digest(R/'freeze.json')==first['freeze_sha256'];assert hull.covers(Point(q.pixel_x,q.pixel_y))==first['inside_control_hull'];compare(m.score(cs,[q],'affine'),first['physical_affine']);compare(m.score(base,[q],'tps'),first['accepted_baseline_tps']);count+=2
for v in read(R/'crop-provenance.json')+read(E/'crop-provenance.json'):
 assert digest(Path(v['parent_path']))==v['parent_sha256'];o=next(read(p) for p in [R/'observations'/f'IS{i}.json' for i in range(20,34)] if Path(read(p)['source_crop_path']).name==v['derived']);fr=o['source_frame'];parent=v['parent_frame'];box=fr['derived_crop_box'];scale=np.asarray(parent['extent'])/np.asarray(parent['display']);assert np.allclose(np.asarray(parent['origin'])+np.asarray(box[:2])*scale,fr['origin']);assert np.allclose((np.asarray(box[2:])-box[:2])*scale,fr['extent'])
for expected,points in [(read(R/'accuracy-summary.json'),checks),(read(R/'regional-summary.json')['western_coast'],checks[:2]),(read(R/'regional-summary.json')['southwestern_lakes'],checks[2:5]),(read(R/'snapshots/first-five-accuracy.json'),checks[:5]),(read(E/'accuracy-summary.json'),checks[5:7]),(read(R/'snapshots/first-seven-accuracy-summary.json'),checks[:7]),(read(ROAD/'accuracy-summary.json'),checks[7:10]),(read(R/'snapshots/first-ten-accuracy-summary.json'),checks[:10]),(read(EAST/'accuracy-summary.json'),checks[10:])]:
...
for v in read(R/'browser-review.json')['screenshots']+read(R/'browser-review.json')['states']+read(E/'browser-review.json')['screenshots']+read(E/'browser-review.json')['states']+read(ROAD/'browser-review.json')['screenshots']+read(ROAD/'browser-review.json')['states']+read(EAST/'browser-review.json')['screenshots']+read(EAST/'browser-review.json')['states']:assert digest(Path(v['path']))==v['sha256']
...
for v in read(EAST/'warped-review/receipt.json')['reviews']:assert digest(EAST/'warped-review'/v['figure'])==v['figure_sha256'];assert v['source_pixel_alpha']>0
assert len(read(R/'import-verification.json')['results'])==5
inside=[p.label for p in checks if hull.covers(Point(p.pixel_x,p.pixel_y))];assert inside==['IS31']
status=read(R/'status.json');assert not status['no_tuning_after_checks'] and status['first_phase_fit_unchanged'] and status['collection_closed'] and status['first_validation_check_count']==14 and status['fresh_checks_after_model_selection']==0 and not status['geographic_acceptance'];compare(status['physical_affine'],read(R/'accuracy-summary.json')['physical_affine'])
print(json.dumps(dict(metric_sets_replayed=count,first_validation_observations_audited=14,derived_crop_frames_verified=3,reference_features_by_file={Path(k).name:len(v) for k,v in reference_cache.items()},original_lake_exteriors_verified=3,inside_control_hull=inside,outside_control_hull_count=13,retained_affine_and_raster_unchanged=True,accepted_baseline_inputs_unchanged=True,actual_raster_windows=14,editable_inventories=5,first_five_inputs_unchanged=True,first_seven_inputs_unchanged=True,first_ten_inputs_unchanged=True,transport_source_id_sets_verified=4,geographic_acceptance=False),indent=2))
