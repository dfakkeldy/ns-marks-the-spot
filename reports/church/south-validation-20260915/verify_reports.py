...
R=Path(__file__).resolve().parent;E=R/'northern-expansion';ROOT=R.parents[2];O=R.parent/'physical-review-20260913/inverness-south';s=importlib.util.spec_from_file_location('score',R.parent/'target-refinement-20260913/score_models.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
...
cs=load_gcps(R/'controls.csv');base=load_gcps(ROOT/'tools/church/gcps/inverness-south.csv');rows=load_gcps(R/'fresh-validation.csv');assert [p for p in rows if p.role=='control']==cs;checks=[p for p in rows if p.role=='check'];assert [p.label for p in checks]==[f'IS{i}' for i in range(20,27)];assert not {p.label for p in checks}&set(f['prior_selection_features']);assert len({(p.lon,p.lat) for p in cs+checks})==11
...
for expected,points in [(read(R/'accuracy-summary.json'),checks),(read(R/'regional-summary.json')['western_coast'],checks[:2]),(read(R/'regional-summary.json')['southwestern_lakes'],checks[2:5]),(read(R/'snapshots/first-five-accuracy.json'),checks[:5]),(read(E/'accuracy-summary.json'),checks[5:])]:
...
for v in read(R/'browser-review.json')['screenshots']+read(R/'browser-review.json')['states']+read(E/'browser-review.json')['screenshots']+read(E/'browser-review.json')['states']:assert digest(Path(v['path']))==v['sha256']
...
status=read(R/'status.json');assert status['no_tuning_after_checks'] and status['fresh_check_count']==7 and not status['geographic_acceptance'];compare(status['physical_affine'],read(R/'accuracy-summary.json')['physical_affine'])
print(json.dumps(dict(metric_sets_replayed=count,fresh_observations_audited=7,derived_crop_frames_verified=3,reference_features_by_file={Path(k).name:len(v) for k,v in reference_cache.items()},original_lake_exteriors_verified=3,all_checks_outside_control_hull=True,retained_affine_and_raster_unchanged=True,accepted_baseline_inputs_unchanged=True,actual_raster_windows=7,editable_inventories=3,first_five_inputs_unchanged=True,geographic_acceptance=False),indent=2))
