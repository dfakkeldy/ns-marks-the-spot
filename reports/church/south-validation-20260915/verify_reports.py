...
R=Path(__file__).resolve().parent;ROOT=R.parents[2];O=R.parent/'physical-review-20260913/inverness-south';s=importlib.util.spec_from_file_location('score',R.parent/'target-refinement-20260913/score_models.py');m=importlib.util.module_from_spec(s);s.loader.exec_module(m)
...
cs=load_gcps(R/'controls.csv');base=load_gcps(ROOT/'tools/church/gcps/inverness-south.csv');rows=load_gcps(R/'fresh-validation.csv');assert [p for p in rows if p.role=='control']==cs;checks=[p for p in rows if p.role=='check'];assert [p.label for p in checks]==[f'IS{i}' for i in range(20,25)];assert not {p.label for p in checks}&set(f['prior_selection_features']);assert len({(p.lon,p.lat) for p in cs+checks})==9
...
for v in read(R/'crop-provenance.json'):
 assert digest(Path(v['parent_path']))==v['parent_sha256'];o=next(read(p) for p in (R/'observations').glob('*.json') if Path(read(p)['source_crop_path']).name==v['derived']);fr=o['source_frame'];parent=v['parent_frame'];box=fr['derived_crop_box'];scale=np.asarray(parent['extent'])/np.asarray(parent['display']);assert np.allclose(np.asarray(parent['origin'])+np.asarray(box[:2])*scale,fr['origin']);assert np.allclose((np.asarray(box[2:])-box[:2])*scale,fr['extent'])
for expected,points in [(read(R/'accuracy-summary.json'),checks),(read(R/'regional-summary.json')['western_coast'],checks[:2]),(read(R/'regional-summary.json')['southwestern_lakes'],checks[2:])]:
...
assert (R/'snapshots/first-five-accuracy.json').read_bytes()==(R/'accuracy-summary.json').read_bytes()
...
for v in read(R/'browser-review.json')['screenshots']+read(R/'browser-review.json')['states']:assert digest(Path(v['path']))==v['sha256']
assert len(read(R/'import-verification.json')['results'])==2
status=read(R/'status.json');assert status['no_tuning_after_checks'] and status['fresh_check_count']==5 and not status['geographic_acceptance'];compare(status['physical_affine'],read(R/'accuracy-summary.json')['physical_affine'])
print(json.dumps(dict(metric_sets_replayed=count,fresh_observations_audited=5,derived_crop_frames_verified=2,original_reference_features=2406,original_lake_exteriors_verified=2,all_checks_outside_control_hull=True,retained_affine_and_raster_unchanged=True,accepted_baseline_inputs_unchanged=True,actual_raster_windows=5,editable_inventories=2,geographic_acceptance=False),indent=2))
