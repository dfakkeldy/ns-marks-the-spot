...
receipt=json.loads((H.parent/'cape-breton-regional-20260915/selected-tps11/artifact-receipt.json').read_text())
assert hashlib.sha256(R.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(H/'controls.csv');control_ids={p.label for p in cs}
obs={}
for path in (H/'observations').glob('CB*.json'):
...
for p,pred in zip(points,preds,strict=True):
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh after TPS11 freeze' if p['id'] in ['CB21','CB22','CB23'] else 'selection diagnostic'
