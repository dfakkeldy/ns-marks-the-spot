...
cs=load_gcps(R/'controls.csv');assert len(cs)==13;assert (R/'controls.csv').read_bytes()==(ROOT/f['inputs']['controls']['path']).read_bytes();assert sha(Path(f['raster']))==f['raster_sha256'];prior=[p for p in load_gcps(ROOT/f['inputs']['prior_inventory']['path']) if p.role=='check'];assert len(prior)==14
...
for ident in ['IS48','IS49']:
 ...
 for q in o['reference_vertices']:
  assert fs[q['feature_id']]['geometry']['coordinates'][q['vertex']][:2]==o['lonlat']
 if ident=='IS48':
  q=o['reference_vertices'][0];assert len(o['reference_vertices'])==1 and fs[q['feature_id']]['properties']['FEAT_CODE']=='WACO20';coords=fs[q['feature_id']]['geometry']['coordinates'];assert coords[q['vertex']][0]<=coords[q['vertex']-1][0] and coords[q['vertex']][0]<=coords[q['vertex']+1][0]
  candidates=read(R/'search-evidence/green-coast-vertices.json');assert min(candidates,key=lambda v:v['lonlat'][0])['lonlat']==o['lonlat']
 else:
  assert len(o['reference_vertices'])==3;assert [fs[q['feature_id']]['properties']['FEAT_CODE'] for q in o['reference_vertices']]==['WARV50','WACO20','WACO20']
...
assert load_gcps(R/'fresh-validation.csv')==cs+prior+new;assert load_gcps(R/'new-checks.csv')==cs+new;assert len({(p.lon,p.lat) for p in cs+prior+new})==29;assert inside==['IS48']
...
ov=read(R/'coverage-overview.json');assert sha(R/'coverage-overview.jpg')==ov['figure_sha256'] and ov['raster_sha256']==f['raster_sha256'] and len(ov['points'])==29
...
status=read(R/'status.json');assert status['fresh_check_count']==16 and status['no_tuning_after_checks'] and not status['geographic_acceptance'];compare(status['physical_tps13'],a['physical_tps13'])
print(json.dumps(dict(metric_sets_replayed=count,new_physical_checks=2,fresh_checks=16,original_coast_extremum_and_stream_mouth_verified=True,inside_control_hull=inside,outside_control_hull=['IS49'],first_fourteen_and_frozen_fit_preserved=True,actual_raster_windows=2,editable_inventories=3,browser_terrain_verified=True,geographic_acceptance=False),indent=2))
