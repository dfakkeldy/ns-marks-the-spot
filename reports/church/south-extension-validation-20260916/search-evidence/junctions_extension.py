...
r=Path('/Users/dfakkeldy/Downloads/church-south-extension-validation-20260916');d=json.load(open('/Users/dfakkeldy/Downloads/church-south-extension-validation-20260916/extension-water-lines.geojson'));nodes=defaultdict(list)
for f in d['features']:
...
for ll,fs in nodes.items():
 ...
 if len(fs)>=3 and len(names)>=1 and -61.43<ll[0]<-61.27 and 45.62<ll[1]<45.71:rows.append(dict(lonlat=ll,branches=fs))
...
(r/'all-junction-search.json').write_text(json.dumps(rows,indent=2));print(json.dumps(rows,indent=2))
