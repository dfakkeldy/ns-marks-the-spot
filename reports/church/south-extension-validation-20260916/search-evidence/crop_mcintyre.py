...
x,y,w,h,dw,dh=21500,30750,1500,1500,1500,1500
v=gdal.Translate('',src,srcWin=[x,y,w,h],width=dw,height=dh,format='MEM');v.ReadAsArray();gdal.Translate(str(r/'mcintyre-native.jpg'),v,format='JPEG');(r/'mcintyre-native-frame.json').write_text(json.dumps(dict(county='inverness',source=src,origin=[x,y],extent=[w,h],display=[dw,dh],rotation=0)));print('Extension crop complete',flush=True)
