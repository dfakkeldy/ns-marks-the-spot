...
x,y,w,h,dw,dh=20200,30900,1600,1600,1600,1600
v=gdal.Translate('',src,srcWin=[x,y,w,h],width=dw,height=dh,format='MEM');v.ReadAsArray();gdal.Translate(str(r/'north-little-native.jpg'),v,format='JPEG');(r/'north-little-native-frame.json').write_text(json.dumps(dict(county='inverness',source=src,origin=[x,y],extent=[w,h],display=[dw,dh],rotation=0)));print('Extension crop complete',flush=True)
