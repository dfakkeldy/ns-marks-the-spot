...
def main():
    ...
    csv=HERE/'controls.csv'
    boundary=HERE.parent/'content-boundary.json'
    ...
    assert len(controls)==7 and all(p.role=='control' for p in controls)
    ...
    cutline=args.out/'affine14-cutline.geojson'
    ...
    vrt=args.out/'affine14.vrt'
    run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Victoria northwest 7-control affine; complete review content; no geographic acceptance.',*build_gcp_arguments(controls),args.source.resolve(),vrt)
    tree=ET.parse(vrt)
    root=tree.getroot()
    ...
    tree.write(vrt)
    raster=args.out/'victoria-northwest-affine7-review-20m.tif'
