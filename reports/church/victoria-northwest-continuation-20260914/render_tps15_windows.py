"""Inspect actual TPS15 pixels against original water geometry at measured features."""
...
RASTER=CACHE/'tps15-rendered/victoria-northwest-tps15-review-20m.tif'
receipt=json.loads((CACHE/'tps15-rendered/artifact-receipt.json').read_text());assert hashlib.sha256(RASTER.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(HERE/'southwestern-support/controls.csv');control_ids={p.label for p in cs};obs={p['id']:p for p in json.loads((HERE/'north-pond-audit/observations.json').read_text())['points']}
for f in (HERE/'observations').glob('VN*.json'):
 p=json.loads(f.read_text());obs[p['id']]=p
ids=['VN11','VN15','VN16','VN17','VN18','VN19','VN20','VN21','VN22','VN23','VN24','VN25','VN26'];points=[obs[id] for id in ids]
...
gdal.UseExceptions();ds=gdal.Open(str(RASTER));inverse=gdal.InvGeoTransform(ds.GetGeoTransform());out=HERE/'tps15-warped-review';out.mkdir(exist_ok=True)
...
for p,pred in zip(points,predictions):
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh at TPS15 freeze' if p['id'] in ['VN25','VN26'] else 'diagnostic'
 d.rectangle((0,1000,1000,1040),fill='#fffdf5');d.text((10,1007),f"{p['id']} - {role} - actual TPS15 / original NSTDB - 2000 ground-metre window",fill='black');d.text((10,1023),'Orange: warped source. Red: reference. Rumsey / Stanford - CC BY-NC-SA 3.0.',fill='black');dest=out/f"{p['id']}.jpg";im.convert('RGB').save(dest,quality=92)
