...
def main():
    ...
    csv=HERE/'affine14-controls.csv'
    boundary=HERE.parents[1]/'physical-review-20260912/richmond/content-boundary.json'
    ...
    run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Richmond 14-control affine; complete review content; no geographic acceptance.',*build_gcp_arguments(controls),args.source.resolve(),vrt)
    tree=ET.parse(vrt)
    root=tree.getroot()
    ...
    tree.write(vrt)
    raster=args.out/'richmond-affine14-20m.tif'
