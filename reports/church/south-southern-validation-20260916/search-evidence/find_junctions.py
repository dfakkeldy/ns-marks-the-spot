...
r=Path('/Users/dfakkeldy/Downloads/church-south-southern-validation-20260916');d=json.load(open('/Users/dfakkeldy/Downloads/church-south-validation-20260915/southwest-water-lines.geojson'));nodes=defaultdict(list)
for f in d['features']:
...
for ll,fs in nodes.items():
 ...
 if len(fs)>=3 and len(names)>=2 and -61.57<ll[0]<-61.31 and 45.72<ll[1]<45.96:rows.append(dict(lonlat=ll,branches=fs))
...
(r/'named-junction-search.json').write_text(json.dumps(rows,indent=2));print(json.dumps(rows,indent=2))
