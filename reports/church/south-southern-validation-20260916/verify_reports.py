...
cs=load_gcps(R/'controls.csv');assert len(cs)==13;assert (R/'controls.csv').read_bytes()==(ROOT/f['inputs']['controls']['path']).read_bytes();assert sha(Path(f['raster']))==f['raster_sha256'];prior=[p for p in load_gcps(ROOT/f['inputs']['prior_inventory']['path']) if p.role=='check'];assert len(prior)==11
...
for ident in ['IS45','IS46']:
 ...
 assert len({q['feature_id'] for q in o['reference_vertices']})==3
...
assert load_gcps(R/'fresh-validation.csv')==cs+prior+new;assert load_gcps(R/'new-check.csv')==cs+new;assert len({(p.lon,p.lat) for p in cs+prior+new})==26;assert inside==['IS45','IS46']
...
assert load_gcps(R/'first-twelve.csv')==cs+prior+[new[0]]
assert load_gcps(R/'first-new-check.csv')==cs+[new[0]]
a12=read(R/'first-twelve-summary.json');compare(m.score(cs,prior+[new[0]],'tps'),a12['physical_tps13']);compare(m.score(base,prior+[new[0]],'tps'),a12['accepted_baseline_tps']);count+=2
...
ov=read(R/'coverage-overview.json');assert sha(R/'coverage-overview.jpg')==ov['figure_sha256'] and ov['raster_sha256']==f['raster_sha256'] and len(ov['points'])==26
...
status=read(R/'status.json');assert status['fresh_check_count']==13 and status['no_tuning_after_checks'] and not status['geographic_acceptance'];compare(status['physical_tps13'],a['physical_tps13'])
print(json.dumps(dict(metric_sets_replayed=count,new_physical_checks=2,fresh_checks=13,original_confluences_verified=True,inside_control_hull=inside,outside_control_hull=[],first_eleven_and_frozen_fit_preserved=True,actual_raster_windows=2,editable_inventories=3,browser_terrain_verified=True,geographic_acceptance=False),indent=2))
