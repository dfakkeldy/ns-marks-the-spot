...
def main():
    ...
    freeze=json.loads((HERE/'affine3-freeze.json').read_text())
    csv=HERE/'affine3-controls.csv'
    ...
    assert len(controls)==3 and all(p.role=='control' for p in controls)
    ...
    cutline=args.out/'main-affine3-cutline.geojson'
    ...
    vrt=args.out/'main-affine3.vrt'
    run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Cape Breton main 3-control affine; complete review content; no geographic acceptance.',*build_gcp_arguments(controls),args.source.resolve(),vrt)
    tree=ET.parse(vrt)
    root=tree.getroot()
    ...
    tree.write(vrt)
    raster=args.out/'cape-breton-affine3-review-20m.tif'
