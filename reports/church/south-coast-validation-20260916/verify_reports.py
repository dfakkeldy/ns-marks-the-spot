...
cs=load_gcps(R/'controls.csv');assert len(cs)==13;assert (R/'controls.csv').read_bytes()==(ROOT/f['inputs']['controls']['path']).read_bytes();assert sha(Path(f['raster']))==f['raster_sha256'];prior=[p for p in load_gcps(ROOT/f['inputs']['prior_inventory']['path']) if p.role=='check'];assert len(prior)==9
...
for ident in ['IS43','IS44']:
 ...
 fs=refs[key]
 if 'reference_banks' in o:
  for b in o['reference_banks']:
   q=b['river'];assert fs[q['feature_id']]['geometry']['coordinates'][q['vertex']][:2]==b['lonlat'];assert fs[q['feature_id']]['properties']['FEAT_CODE'] in ['WARV10','WARV20']
   for q in b['coast']:assert fs[q['feature_id']]['geometry']['coordinates'][q['vertex']][:2]==b['lonlat'];assert fs[q['feature_id']]['properties']['FEAT_CODE']=='WACO20'
  assert np.allclose(np.mean([b['lonlat'] for b in o['reference_banks']],axis=0),o['lonlat'],atol=1e-12,rtol=0)
 else:
  for q in o['reference_vertices']:
   coords=fs[q['feature_id']]['geometry']['coordinates'];assert coords[q['vertex']][:2]==o['lonlat'];assert fs[q['feature_id']]['properties']['FEAT_CODE']=='WACO20';assert coords[q['vertex']][1]>=coords[q['vertex']-1][1] and coords[q['vertex']][1]>=coords[q['vertex']+1][1]
...
assert load_gcps(R/'fresh-validation.csv')==cs+prior+new;assert load_gcps(R/'new-checks.csv')==cs+new;assert len({(p.lon,p.lat) for p in cs+prior+new})==24;assert inside==['IS43']
...
ov=read(R/'coverage-overview.json');assert sha(R/'coverage-overview.jpg')==ov['figure_sha256'] and ov['raster_sha256']==f['raster_sha256'] and len(ov['points'])==24
...
status=read(R/'status.json');assert status['fresh_check_count']==11 and status['no_tuning_after_checks'] and not status['geographic_acceptance'];compare(status['physical_tps13'],a['physical_tps13'])
print(json.dumps(dict(metric_sets_replayed=count,new_physical_checks=2,fresh_checks=11,river_coast_midpoint_verified=True,northern_apex_verified=True,inside_control_hull=inside,outside_control_hull=['IS44'],first_nine_and_frozen_fit_preserved=True,actual_raster_windows=2,editable_inventories=3,browser_terrain_verified=True,geographic_acceptance=False),indent=2))
