...
C=Path('/Users/dfakkeldy/Downloads/church-cape-breton-interior-20260915')
R=C/'tps6-rendered/cape-breton-tps6-review-20m.tif'
receipt=json.loads((H/'selected-tps6/artifact-receipt.json').read_text())
assert hashlib.sha256(R.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(H/'selected-tps6/controls.csv');control_ids={p.label for p in cs}
...
for p,pred in zip(points,preds,strict=True):
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh after TPS6 freeze' if p['id'] in ['CB13'] else 'selection diagnostic'
 d.rectangle((0,1000,1000,1040),fill='#fffdf5');d.text((10,1007),f"{p['id']} - {role} - actual TPS6 / original NSTDB - 2000 ground-metre window",fill='black');d.text((10,1023),'Orange: warped source. Red: reference. Rumsey / Stanford - CC BY-NC-SA 3.0.',fill='black');dest=out/f"{p['id']}.jpg";im.convert('RGB').save(dest,quality=92)
