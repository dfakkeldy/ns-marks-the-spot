...
def main():
 ...
 cs=load_gcps(HERE/'controls.csv');assert len(cs)==5 and all(p.role=='control' for p in cs);args=build_gcp_arguments(cs);boundary=HERE.parent/'content-boundary.json';assert digest(boundary)==freeze['boundary_sha256'];ring=json.loads(boundary.read_text())['ring_pixel_xy'];poly=Cutline(tuple(map(tuple,ring)))
 ...
 cut=a.out/'cape-breton-tps5-cutline.geojson';cut.write_text(cutline_geojson(transform(densify(poly,250)).tolist(),3857));vrt=a.out/'cape-breton-tps5.vrt'
 run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Cape Breton main; five physical controls; TPS; no geographic acceptance.',*args,a.source.resolve(),vrt)
 raster=a.out/'cape-breton-tps5-review-20m.tif'
