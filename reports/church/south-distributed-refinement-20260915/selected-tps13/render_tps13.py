"""Render the frozen thirteen-control TPS and audit sampled orientation."""
...
SOURCE_SHA='37021ed086f7bbce542b519e9a74242acc5b53ed1944880468f6f91d6234a7f8'
def digest(p):
...
def main():
 ...
 cs=load_gcps(HERE/'controls.csv');assert len(cs)==13 and all(p.role=='control' for p in cs);args=build_gcp_arguments(cs);boundary=HERE.parent/'content-boundary.json';assert digest(boundary)==freeze['boundary_sha256'];ring=json.loads(boundary.read_text())['ring_pixel_xy'];poly=Cutline(tuple(map(tuple,ring)))
 ...
 cut=a.out/'inverness-south-tps13-cutline.geojson';cut.write_text(cutline_geojson(transform(densify(poly,250)).tolist(),3857));vrt=a.out/'inverness-south-tps13.vrt'
 run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection / Stanford Libraries; recorded CC BY-NC-SA 3.0','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Church Inverness south; thirteen physical controls; TPS; no geographic acceptance.',*args,a.source.resolve(),vrt)
 raster=a.out/'inverness-south-tps13-review-20m.tif'
