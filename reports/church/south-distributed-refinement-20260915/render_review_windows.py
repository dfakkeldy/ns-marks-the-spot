...
R=Path(__file__).resolve().parent;E=R/'selected-tps13';C=Path('/Users/dfakkeldy/Downloads/church-south-road-validation-20260915');receipt=json.loads((E/'local-artifact.json').read_text());raster=Path('/Users/dfakkeldy/Downloads/church-south-distributed-refinement-20260915/rendered/inverness-south-tps13-review-20m.tif');freeze={'raster_sha256':'b849ad07dd0d61dcb8484e257090bc19721cb995fa608c41874f4c13ee237040'};assert hashlib.sha256(raster.read_bytes()).hexdigest()==freeze['raster_sha256'];cs=load_gcps(E/'controls.csv')
rows=load_gcps(R/'thirteen-controls/diagnostics.csv')
points=[dict(id=p.label,pixel_xy=[p.pixel_x,p.pixel_y],lonlat=[p.lon,p.lat],role=p.role) for p in rows]
features=[];refs=[]
