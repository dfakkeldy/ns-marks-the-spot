...
C=Path('/Users/dfakkeldy/Downloads/church-cape-breton-regional-20260915')
R=C/'rendered/cape-breton-tps11-review-20m.tif'
receipt=json.loads((H/'selected-tps11/artifact-receipt.json').read_text())
assert hashlib.sha256(R.read_bytes()).hexdigest()==receipt['output_sha256']
cs=load_gcps(H/'selected-tps11/controls.csv');control_ids={p.label for p in cs}
obs={p['id']:p for p in json.loads((H.parent/'physical-review-20260913/cape-breton/observations.json').read_text())['points']}
obs.update({p['id']:p for p in json.loads((H.parent/'distributed-review-20260913/cape-breton/observations.json').read_text())['points']})
for p in (H/'observations').glob('CB*.json'):
 o=json.loads(p.read_text());obs[o['id']]=o

for path in (H.parent/'cape-breton-continuation-20260915/observations').glob('CB*.json'):
 o=json.loads(path.read_text());obs[o['id']]=o
for path in (H.parent/'cape-breton-interior-20260915/observations').glob('CB*.json'):
 o=json.loads(path.read_text());obs[o['id']]=o
for path in (H.parent/'cape-breton-northern-20260915/observations').glob('CB*.json'):
 o=json.loads(path.read_text());obs[o['id']]=o
points=[obs[k] for k in sorted(obs)]
refs=[Path('/Users/dfakkeldy/Downloads/church-review-20260913/cape-water-lines.geojson')]
fs={}
for path in refs:
...
for p,pred in zip(points,preds,strict=True):
 lon,lat=p['lonlat'];x,y=lonlat_to_mercator(lon,lat);half_ground=1000;half=half_ground/math.cos(math.radians(lat));bounds=[x-half,y-half,x+half,y+half]
 ...
 role='CONTROL - not validation' if p['id'] in control_ids else 'fresh after TPS11 freeze' if p['id'] in ['CB18','CB19','CB20'] else 'selection diagnostic'
 d.rectangle((0,1000,1000,1040),fill='#fffdf5');d.text((10,1007),f"{p['id']} - {role} - actual TPS11 / original NSTDB - {half_ground*2} ground-metre window",fill='black');d.text((10,1023),'Orange: warped source. Red: reference. Rumsey / Stanford - CC BY-NC-SA 3.0.',fill='black');dest=out/f"{p['id']}.jpg";im.convert('RGB').save(dest,quality=92)
