"""Render the six-control Richmond TPS from the exact archival TIFF.
...
def main():
 ...
 points=load_gcps(HERE/'richmond-refined.csv');controls=[p for p in points if p.role=='control'];assert len(controls)==6;gcp=build_gcp_arguments(controls);ring=json.loads((HERE.parent/'content-boundary.json').read_text())['ring_pixel_xy'];poly=Cutline(tuple(map(tuple,ring)))
 def transform(p):
 ...
 dense=densify(poly,250.0);world=transform(dense).tolist();world.append(world[0]);cut=a.out/'richmond-refined-cutline.geojson';cut.write_text(json.dumps({'type':'FeatureCollection','crs':{'type':'name','properties':{'name':'EPSG:3857'}},'features':[{'type':'Feature','properties':{},'geometry':{'type':'Polygon','coordinates':[world]}}]},indent=2)+'\n')
 vrt=a.out/'richmond-refined.vrt';run('gdal_translate','-q','-of','VRT','-a_srs','EPSG:3857','-mo','TIFFTAG_COPYRIGHT=David Rumsey Map Collection, David Rumsey Map Center, Stanford Libraries; CC BY-NC-SA 3.0; https://www.davidrumsey.com/about/copyright-and-permissions','-mo','TIFFTAG_IMAGEDESCRIPTION=PROVISIONAL Richmond County Church map: six physical controls, TPS, expanded crop excluding insets. Diagnostic accuracy only; not accepted for publication.','-mo','SOURCE_URL=https://www.davidrumsey.com/luna/servlet/iiif/m/RUMSEY~8~1~373669~90140407/manifest',*gcp,a.source.resolve(),vrt)
 full=a.out/'richmond-refined-tps-5m.tif';review=a.out/'richmond-refined-tps-20m.tif'
 for p,res in [(review,20),(full,5)]:
  ...
  if res==20:run('gdal_translate','-q','-of','PNG','-outsize','1800','0',p,a.out/'richmond-refined-preview.png')
 outputs=[]
 ...
 receipt=dict(source_path=str(a.source.resolve()),source_sha256=SHA,csv_sha256=digest(HERE/'richmond-refined.csv'),boundary_sha256=digest(HERE.parent/'content-boundary.json'),new_observations_sha256=digest(HERE/'new-observations.json'),transform='GDAL TPS -et 0',crs='EPSG:3857',control_count=6,diagnostic_check_count=6,fresh_validation_count=0,geographic_acceptance=False,orientation=orientation,outputs=outputs)
