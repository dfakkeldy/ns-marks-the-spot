...
const path='reports/church/physical-review-20260912/richmond/refinement-04/richmond-refined-v4.csv';
const parsed=parseFletcherGcps(readFileSync(path,'utf8'),{pixelSize:{width:35735,height:30429}});
assert.equal(parsed.gcps.length,10);assert.equal(parsed.checks.length,19);
assert.deepEqual(parseFletcherGcps(serializeFletcherGcps(parsed)).rows,parsed.rows);
for(const [name,count] of [['frozen-fit.csv',13],['fresh-validation-review.csv',6]] as const){const p=parseFletcherGcps(readFileSync('reports/church/physical-review-20260912/richmond/refinement-04/'+name,'utf8'));assert.equal(p.gcps.length,10);assert.equal(p.checks.length,count);assert.deepEqual(parseFletcherGcps(serializeFletcherGcps(p)).rows,p.rows);}
...
console.log(JSON.stringify({controls:10,checks:19,semantic_csv_roundtrip:true,maximum_web_gdal_difference_projected_m:difference,scope:'Parser and TPS computation only; not raw-scan browser mesh or geographic acceptance'},null,2));
