...
const root='reports/church/richmond-south-seam-20260915/';
...
const results=[];
for(const [file,nchecks] of [['richmond-controls.csv',0],['richmond-new-validation.csv',2],['richmond-cumulative-validation.csv',16]] as const){
 const p=parseFletcherGcps(readFileSync(root+file,'utf8'),{pixelSize:{width:35735,height:30429}});
 assert.equal(p.gcps.length,14);assert.equal(p.checks.length,nchecks);assert.deepEqual(parseFletcherGcps(serializeFletcherGcps(p)).rows,p.rows);
 ...
 assert.ok(worst<.001);results.push({file,controls:14,checks:nchecks,semanticRoundtrip:true,comparisons:[{method:'affine',maxDifferenceProjectedMetres:worst}]});
}
console.log(JSON.stringify({results,scope:'Production CSV parser and frozen Richmond affine solver at fresh checks and content vertices; not raw-scan browser mesh acceptance.'},null,2));
