...
cache={}
for path in sorted((R/'observations').glob('R*.json')):
 o=json.loads(path.read_text());key=o['reference_path']
 if key not in cache:cache[key]=json.loads(Path(key).read_text())['features']
 lon,lat=o['lonlat'];c=math.cos(math.radians(lat));span=3200 if o['id']=='R55' else 1600;im=Image.new('RGB',(1000,1040),'white');d=ImageDraw.Draw(im)
 ...
 d.line((480,500,520,500),fill='red',width=3);d.line((500,480,500,520),fill='red',width=3);d.rectangle((0,1000,1000,1040),fill='white');d.text((10,1006),o['id']+f' exact reference measurement; {span} ground-metre window; north up',fill='black');d.text((10,1022),'Original NSTDB. Red +: adopted coordinate; teal: selected exterior; blue: context.',fill='black');im.save(R/'observations'/(o['id']+'-reference-review.jpg'),quality=94)
