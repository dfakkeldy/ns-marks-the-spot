...
receipt=json.loads((HERE/'../../coverage-20260914/richmond/affine14-artifact-receipt.json').read_text())
assert hashlib.sha256(RASTER.read_bytes()).hexdigest()==receipt['output_sha256']
config=json.loads((HERE/'../../coverage-20260914/richmond/terrain-preview-inputs.json').read_text())
features={}
config['water_inputs'].append(dict(path='/Users/dfakkeldy/Downloads/church-continuation-20260914/southern-water.geojson',sha256='5f869c94c7048dd20b85a7efa01f01975eac66c231942fbdabe5e15812aada17'))
...
fit=solve_affine(load_gcps(HERE/'../../coverage-20260914/richmond/affine14-controls.csv'))
source=gdal.Open(str(RASTER))
inverse=gdal.InvGeoTransform(source.GetGeoTransform())
out=HERE/'warped-review'
out.mkdir(exist_ok=True)
reviews=[]
for path in sorted((HERE/'observations').glob('R*.json')):
