...
const root='reports/church/inverness-north-continuation-20260914/';
const results=[];
for(const [file,checks] of [['controls.csv',0],['diagnostic-review.csv',2],['fresh-validation.csv',1]] as const){
 const p=parseFletcherGcps(readFileSync(root+'north-tip-trial/'+file,'utf8'),{pixelSize:{width:34427,height:34543}});
 assert.equal(p.gcps.length,5);assert.equal(p.checks.length,checks);assert.deepEqual(parseFletcherGcps(serializeFletcherGcps(p)).rows,p.rows);
 ...
 assert.ok(worst<.001);results.push({file,controls:5,checks,semanticRoundtrip:true,maxTpsDifferenceProjectedMetres:worst});
}
