...
prior_rows=load_gcps(ROOT/f['inputs']['prior_inventory']['path']);prior=[p for p in prior_rows if p.role=='check'];assert len(prior)==8;assert (R/'fresh-validation.csv').read_bytes().startswith((ROOT/f['inputs']['prior_inventory']['path']).read_bytes())
o=read(R/'observations/IS42.json');fr=o['source_frame'];assert fr['rotation']==0;assert sha(Path(o['source_crop_path']))==o['source_crop_sha256'];xy=np.asarray(fr['origin'])+np.asarray(o['source_display_pixel_xy'])*np.asarray(fr['extent'])/np.asarray(fr['display']);assert np.allclose(xy,o['pixel_xy'],atol=1e-9,rtol=0)
...
p=GroundControlPoint(*o['pixel_xy'],*o['lonlat'],'check','IS42');assert load_gcps(R/'fresh-validation.csv')==cs+prior+[p];assert load_gcps(R/'new-check.csv')==cs+[p];assert not {(p.lon,p.lat)}&{(v.lon,v.lat) for v in cs+prior};assert p.label not in {v.label for v in cs+prior}
base=load_gcps(ROOT/f['inputs']['baseline_controls']['path']);first=read(R/'IS42-first.json');assert first['observation_sha256']==sha(R/'observations/IS42.json') and first['freeze_sha256']==sha(R/'freeze.json');assert first['scored_at']>=o['recorded_at'];assert first['inside_control_hull']==MultiPoint([(q.pixel_x,q.pixel_y) for q in cs]).convex_hull.covers(Point(*xy))
...
ov=read(R/'coverage-overview.json');assert sha(R/'coverage-overview.jpg')==ov['figure_sha256'] and ov['raster_sha256']==f['raster_sha256'] and len(ov['points'])==22
...
state=next(Path(v['path']).read_text() for v in browser['states'] if Path(v['path']).name=='IS42-import-state.txt');assert 'checkbox "inverness-south-tps13-review-20m" [checked]' in state and '4,751×6,488' in state
state=next(Path(v['path']).read_text() for v in browser['states'] if Path(v['path']).name=='IS42-terrain10x-state.txt');assert 'status: 10×' in state
...
status=read(R/'status.json');assert status['fresh_check_count']==9 and status['no_tuning_after_checks'] and not status['geographic_acceptance'];compare(status['physical_tps13'],read(R/'accuracy-summary.json')['physical_tps13'])
print(json.dumps(dict(metric_sets_replayed=6,new_physical_check='IS42',fresh_checks=9,native_water_junction_verified=True,original_reference_vertices_verified=True,first_eight_and_frozen_fit_preserved=True,actual_raster_window_verified=True,editable_inventories=3,browser_2d_terrain_verified=True,geographic_acceptance=False),indent=2))
