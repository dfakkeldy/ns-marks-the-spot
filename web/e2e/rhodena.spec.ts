...
for(const width of [1440,390])test(`Rhodena sources, layers and visibility at ${width}px`,async({page})=>{
  ...
  await expect(page.getByRole('combobox',{name:'Viewshed selection'})).toHaveValue('all');
  await expect(page.getByText('All six turbines: combined potential visibility',{exact:true})).toBeVisible();
  const combinedImage=await page.locator('.rhodena-viewshed-raster').getAttribute('src');
  await page.getByRole('combobox',{name:'Viewshed selection'}).selectOption('T6');
  await expect(page.getByText('T6: preliminary 200 m blade-tip visibility',{exact:true})).toBeVisible({timeout:90000});
  expect(await page.locator('.rhodena-viewshed-raster').getAttribute('src')).not.toBe(combinedImage);
  await page.getByRole('combobox',{name:'Viewshed selection'}).selectOption('all');
  await expect(page.getByText('All six turbines: combined potential visibility',{exact:true})).toBeVisible({timeout:90000});
  ...
  await expect(page.locator('.rhodena-visibility-summary')).toBeVisible();
  await page.screenshot({path:`/tmp/rhodena-viewpoint-${width}.png`});
