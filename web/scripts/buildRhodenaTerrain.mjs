...
if(process.argv.includes('--check')){
 ...
 const pinned=JSON.parse(await readFile(new URL('terrain.json',out),'utf8'));
 const expectedTiles=new Map(pinned.tiles.map(tile=>[tile.url,tile.sha256]));
 const z=12,n=2**z;
 ...
 for(let y=north;y<=south;y++)for(let x=west;x<=east;x++){
  ...
  if(sha(bytes)!==expectedTiles.get(url))throw Error(`Terrain source changed: ${url}; review and update the pinned source receipt/date before regenerating`);
 ...
 if(tiles.length!==pinned.tiles.length)throw Error('Terrain tile coverage changed; review the pinned receipt');
