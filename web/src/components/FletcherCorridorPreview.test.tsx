...
function preview(mode: MapRenderMode = 'interactive') {
  return <MapContainer center={[45.74742, -61.4635]} zoom={17}><FletcherCorridorPreview renderMode={mode} /></MapContainer>;
}
describe('corrected corridor review control', () => {
  it('does not expose an unconfigured preview or add imagery to print captures', () => {
    ...
    expect(screen.queryByRole('checkbox')).toBeNull();
    expect(document.querySelector('.fletcher-corridor-tiles')).toBeNull();
  });
  it('removes and restores tiles with its own toggle and cleans up for print', () => {
    vi.stubEnv('VITE_FLETCHER_CORRIDOR_TILE_BASE_URL', 'http://127.0.0.1:4198');
    const view = render(preview());
    const check = screen.getByRole('checkbox', {name:'Fletcher · corrected corridor'});
    expect(document.querySelector('.fletcher-corridor-tiles')).not.toBeNull();
