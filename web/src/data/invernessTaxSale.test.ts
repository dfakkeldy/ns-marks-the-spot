...
describe("the Inverness County 2026 public dataset", () => {
  it("preserves all 45 notice entries while exposing only 27 advertised PIDs", () => {
    expect(taxSaleListings).toHaveLength(45);
    expect(taxSalePids).toHaveLength(27);
    expect(new Set(taxSalePids).size).toBe(27);
    expect(
      taxSaleListings
        .filter(({ listingStatus }) => listingStatus === "withdrawn")
        .map(({ lien }) => lien),
    ).toEqual([5, 6, 8, 9, 10, 11, 12, 13, 14, 15, 28, 33, 34, 36, 37, 40, 41, 45]);
  });
  ...
  it("records the corrected official source and sale details", () => {
    expect(invernessTaxSaleNotice.sourceUrl).toBe(
      "https://invernesscounty.ca/wp-content/uploads/2026/07/Tax-Sale_August-11-11.pdf",
    );
