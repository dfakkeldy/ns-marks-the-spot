  ...
  return <div className="terrain-camera-controls">
    <label htmlFor={`${id}-tilt`}><span>Tilt</span><output>{camera.pitch}°</output>
      <input id={`${id}-tilt`} aria-label="Map tilt" type="range" min="0" max="65" step="1" value={camera.pitch}
        onChange={event => map.jumpTo({ pitch: Number(event.target.value) })} />
    </label>
    <div className="terrain-bearing">Direction <output>{(camera.bearing + 360) % 360}°</output></div>
    <div className="terrain-rotate-buttons">
      <button type="button" aria-label="Rotate left" onClick={() => map.jumpTo({ bearing: map.getBearing() - 30 })}>↶</button>
      <button type="button" onClick={() => map.jumpTo({ bearing: 0 })}>North up</button>
      <button type="button" aria-label="Rotate right" onClick={() => map.jumpTo({ bearing: map.getBearing() + 30 })}>↷</button>
    </div>
