...
export function tilesForBounds(
  ...
  // Using floor() on the max side includes one extra column/row when bounds
  // land exactly on a tile-grid boundary (the min-side floor already picks the
  // correct starting tile). This is intentional: the extra tile draws
  // off-canvas and ensures complete coverage without gaps.
