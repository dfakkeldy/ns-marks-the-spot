...
- Important 4: Added bundled `ProvinceRestrictedGeographicServicesLicense.md`, catalog metadata pointing at that bundled license reference, an info-sheet `Link` when resolved, and tests proving Province layers have a license URL or bundled reference. Follow-up review expanded the bundled document from a short notice into the full local license text and added assertions for the legal sections.
...
- PASS: `xcodebuild test -project ns-marks-the-spot.xcodeproj -scheme ns-marks-the-spot -destination 'platform=iOS Simulator,name=iPhone 17 Pro' -only-testing:ns-marks-the-spotTests/AttributionTests CODE_SIGNING_ALLOWED=NO` after expanding the bundled license document.
