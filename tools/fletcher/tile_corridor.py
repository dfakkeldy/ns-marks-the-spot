"""Tile the accepted Hawkesbury–Mabou raster without refitting or widening it."""
...
ACCEPTANCE = ROOT / "reports/fletcher/southern-seam/browsing-acceptance.json"
REVISION = "fletcher-corridor-20260908.1"
...
def main():
    ...
    accepted = next(
        r
        for r in acceptance["rasters"]
        if r["file"] == "hawkesbury-mabou-browsing-preview.tif"
    )
    if (
        acceptance["status"] != "accepted-for-approximate-corridor-browsing"
        or digest(a.source) != accepted["sha256"]
    ):
        raise ValueError("Source does not match the explicitly accepted corridor")
    ...
    if (
        ...
        raise ValueError("Expected the accepted RGBA raster")
    ...
    source = {
        "revision": REVISION,
        "name": "Fletcher corrected corridor — Port Hawkesbury to Mabou",
