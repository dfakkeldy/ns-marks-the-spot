...
def render(source, fit_path, components_path, out):
    ...
    receipt=dict(source_sha256=digest(source),fit_sha256=digest(fit_path),components_sha256=digest(components_path),components=results)
    (out/'render-receipt.json').write_text(json.dumps(receipt,indent=2)+'\n')
    return receipt

if __name__=='__main__':
    p=argparse.ArgumentParser(description=__doc__);p.add_argument('source',type=Path);p.add_argument('fit',type=Path);p.add_argument('components',type=Path);p.add_argument('out',type=Path);a=p.parse_args();print(json.dumps(render(a.source,a.fit,a.components,a.out),indent=2))
