...
Crown Land (`3nka-59nz`) is a separate bundled display archive in
`web/public/atlas/crown/`, generated by `tools/build_crown_atlas.py`. It unions
all accepted mapped polygons, including full and partial provincial interest,
before generalizing and tiling. Outlines are extracted from the union as lines
before tile clipping: neither shared parcel edges nor artificial tile edges
are outlined. Interior holes, disconnected areas and real gaps are retained.
The receipt preserves source identity, release, download hash, counts,
quarantined records and transformations. Union component IDs are display IDs,
never PIDs or ownership evidence. The existing restricted live Crown Lands
research overlay remains a separate catalogue control.
