...
21, 2026 after the auction. Only official municipal primary sources were used.
Text extraction supported reconciliation; PIDs and financial values in the
fourteen result-backed Halifax notice/result PDFs were also checked against
rendered pages. Tender terms independently confirmed the sale method for six
events. Public data and screenshots omit assessed-owner and bidder names.
...
The included dataset has 87 owner-free records and 93 exact PIDs: 82 sold
...
specific outcome or winning-bid fields enter the layer only after the official
notice/result pair supports deterministic, owner-free reconciliation and an
exact or otherwise unambiguous NSPRD match.
