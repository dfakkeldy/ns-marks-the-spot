...
| PRINT-08 | Failed map layer | Both | Warning, retry, deliberate incomplete print | Pass | Mocked acceptance harness passed: `PrintPreview.test.tsx` requires an explicit incomplete-map decision, exposes retry, retains the warning, and limits receipt layers to those rendered; `PrintDocuments.test.tsx` names the failed layer. |
